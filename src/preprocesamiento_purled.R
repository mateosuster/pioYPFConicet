...
library(Hmisc)
library(zoo)
source("src/functiones_hidrocarburos.R")
...
### TCP
tcp_anual <- read_excel(here("data/tcp/tcp_anual.xlsx"),
...
## IPC
ipc <- read_excel(here("data/indices/ipc_mensual_1963_2018.xlsx")) %>%
...
sipm_serie56_95 <- read_excel(here("data/indec/sipm-serie56-95.xls"),
  skip = 6
) %>%
  rename(
    anio = mes,
...
ipim_base94 <- sipm_serie56_95 %>%
  mutate(ipim_nivel_gral_94 = generar_indice(ipim_nivel_gral,
    fecha = anio,
    fecha_base = 1994
  )) %>%
...
renta_hidrocarburos_jk <- read_excel(here("data/otros autores/renta_hidrocarburos_jk.xls"),
  ...
  mutate_all(as.double) %>%
  rename(
    precio_me_gas = `Gas natural mercado externo`,
  ...
  select(
    anio = indice_tiempo, unidad_gas, unidad_crudo, unidad_renta,
  ...
  mutate_at(
    .vars = c("prod_petcru", "expo_crudo", "crudo_interno"),
    .funs = conversor_m3bbl_q
  ) %>%
  mutate_at(
    .vars = c("precio_mi_crudo", "precio_me_crudo"),
    .funs = conversor_m3bbl_p
  ) %>%
  mutate_at(
    .vars = c("prod_gasnat", "expo_gas", "gas_interno"),
    .funs = conversor_m3MMBTU_q
  ) %>%
  mutate_at(
    .vars = c("precio_mi_gas", "precio_bolivia_gas_pesos"),
...
renta_precio_expo_crudo_jk <- renta_hidrocarburos_jk %>%
  select(
    anio, prod_petcru, expo_crudo, crudo_interno,
...
renta_precio_expo_gas_jk <- renta_hidrocarburos_jk %>%
  select(
    anio, prod_gasnat, expo_gas, gas_interno,
...
renta_hidrocarburos_fr <- read_excel(here("data/otros autores/renta_hidrocarburos_FR.xlsx")) %>%
  select(-unidad) %>%
  rename(
    regalias = "Regalías",
    retenciones = Retenciones,
  ...
  mutate(
    regalias = regalias * ipc_08 / ipc_18,
...
# dachevsky
renta_fd <- read_excel(here("data/otros autores/PonenciaJHISDAchevsky.xlsx")) %>%
  select(anio = "...1", pv_neta = "...3", stock = Stock, tg_normal = "tg industrial") %>%
  slice(-1) %>%
  mutate_all(as.double) %>%
  mutate(renta_total = pv_neta - (stock / 1000 * tg_normal), fuente = "FD")

renta_bil_fr <- renta_hidrocarburos_fr %>%
  rename(
    renta_sobrevaluacion = expo_tc,
    renta_diferencial_precios = dif_precios
  ) %>%
  reshape2::melt(
    id.vars = c("anio", "ipc_18", "unidad"),
    variable.name = "tipo_renta",
    value.name = "valor"
  ) %>%
  left_join(tcp_anual_b) %>%
  mutate(
    autor = "Fárfaro Ruiz y Bil",
    valor = (valor * ipc_18) / tcc,
    unidad = "Millones de USD",
    tipo_renta = as.character(tipo_renta)
  ) %>%
  select(anio, unidad, tipo_de_renta = tipo_renta, valor, autor)

# otros autores
renta_autores <- read_csv(here("data/otros autores/renta_autores.csv"),
  # row.names = F,
  ...
  rbind(renta_bil_fr) %>%
  mutate(
    tipo_de_renta = case_when(
      tipo_de_renta == "subsidio_consumo_total" ~ "subsidios",
      T ~ tipo_de_renta
    ),
    valor = case_when(
      # (tipo_de_renta == "subsidios")&(autor=="Propia")&(anio<2008) ~ NA_real_,
      (tipo_de_renta == "retenciones") &
        (
          # (autor=="Propia")|
          (autor == "Fárfaro Ruiz y Bil")) & (anio < 2002) ~ NA_real_,
      # (tipo_de_renta == "renta_empresas")&(autor=="Propia")&(anio<1997)~NA_real_,
      T ~ valor
    )
  )
...
prod_anuario <- read_excel(here("data/anuario_de_combustibles/Produccion_Desde_1911.xls")) %>%
  rename(
    anio = "AÑO", crudo_anuario_Mm3 = "PETROLEO (Mm3)",
    gas_h = `GAS NATURAL (Mill. m3)`, carbon_h = `CARBON (MTn) (*)`
  ) %>%
  mutate(
    gas_anuario_MMm3 = as.double(gas_h),
...
cuadro_2.1 <- read_excel(here("data/idee/Precios del petroleo crudo y derivados 1970 - 1989.xlsx"),
  skip = 1, sheet = 1
) %>%
  rename(unidad = "...5")

# base Ministerio de Hacienda
hidrocarburos_base_mecon <- read_csv(here("data/mecon/hidrocarburos_produccion.csv"),
...
# Base regalias
prod_regalias_crudo <- read_delim(here("data/secretaria_energia/regalias/produccion_crudo_regalias.csv"),
  ";",
...
prod_sesco_crudo_a <- read_csv(here("data/secretaria_energia/sesco/produccin-de-petrleo-anterior-al-2009.csv")) %>%
  select(anio, mes, idconcepto, concepto, Cantidad) %>%
  rename(cantidad = Cantidad) %>%
  # filter(!idconcepto %in% c(4, 5, 7, 8, 9	)) %>%
  filter(concepto %in% c(
    "Producción Primaria (m3)", "Producción Secundaria (m3)",
...
prod_sesco_crudo_b <- read_csv(here("data/secretaria_energia/sesco/produccin-de-petrleo-por-yacimiento.csv")) %>%
  select(anio, mes, idconcepto, concepto, cantidad) %>%
  # filter(!idconcepto %in% c(4, 5, 7, 8, 9	)) %>%
...
prod_sec_energia <- read.csv(here("data/secretaria_energia/sesco/serie-produccion-petroleo-total-pais-desde-1950.csv")) %>%
  mutate(
    produccion_petroleo = produccion_petroleo * 1000,
    unidad = "m3"
  ) %>%
...
prod_eia_crudo <- read_csv(here("data/eia/oil_production_arg.csv"),
  ...
  rename(
    total_oil_and_other = `        Total petroleum and other liquids (Mb/d)`,
  ...
  mutate_at(
    .vars = (c(
      "crude_oil_condensate", "natural_gas", "other_liquids",
...
# EIA all data
eia_all_data <- read_csv(here("data/eia/INT-Export-04-13-2026_20-17-29.csv"),
  skip = 1
)
...
prod_merge_crudo <- prod_regalias_crudo %>%
  ...
  mutate(
    prod_crudo = case_when(
      anio < 1950 ~ crudo_anuario,
      anio %in% 1950:YEAR_SEC_ENERGIA_UPPER ~ crudo_sec_energia,
      anio > YEAR_SEC_ENERGIA_UPPER ~ crudo_sesco
    ),
    unidad = "barriles",
...
# Existencias
existencias_petroleo <- read_excel(here("data/secretaria_energia/existencias_crudo.xlsx")) %>%
...
# Base regalias
prod_regalias_gas <- read_delim(here("data/secretaria_energia/regalias/produccion_gas_regalias.csv"),
  ";",
  escape_double = FALSE, trim_ws = TRUE
) %>%
  rename(anio = "AÑO") %>%
...
# Base SESCO
# pre 2009
prod_sesco_gas_a <- read_csv(here("data/secretaria_energia/sesco/produccin-de-gas-anterior-al-2009.csv")) %>%
  select(anio, mes, idconcepto, concepto, Cantidad) %>%
  rename(cantidad = Cantidad) %>%
  filter(idconcepto != 4) %>%
  group_by(anio) %>%
...
# post 2009
prod_sesco_gas_b <- read_csv(here("data/secretaria_energia/sesco/produccin-de-gas-por-yacimiento.csv")) %>%
  select(anio, mes, idconcepto, concepto, cantidad) %>%
...
# sesco historic
prod_sec_energia_gas <- read.csv(here("data/secretaria_energia/sesco/producciongasnaturaldesde-1950.csv")) %>%
  mutate(
    produccion_gas_natural = produccion_gas_natural * 1000,
    unidad = "Miles de m3"
  ) %>%
...
# EIA
prod_eia_dry_natural_gas <- read_csv(here("data/eia/Dry_natural_gas_production_Argentina_Annual.csv"),
  skip = 4
) %>%
  rename(
    anio = Year,
  ...
  mutate(
    prod_eia = conversor_ft3m3_q(prod_eia * 1000000),
    unidad = "Miles de m3"
  )
...
vtas_precio_memo_ypf_crudo <- read_excel(here("data/ypf/vtas_valor_cantidad_precio_crudo.xlsx")) %>%
  rename(
    cant_vendidad = "m3", valor_vendido = "valor", precio_a = "$/m3",
  ...
  select(
    anio, unidad_cant, moneda, valor_vendido, cant_vendidad,
...
precios_mecon <- read_excel(here("data/mecon/base-mineria-e-hidrocarburos cuentas nacionales.xls"),
  ...
  filter(!is.na(anio) & !(anio %in% c(2002:2003))) %>%
  mutate_all(as.double) %>%
  group_by(anio) %>%
  summarise(
    precio_gas_me_pesos_Mm3 = mean(precio_gas_me_pesos_Mm3, na.rm = T),
  ...
  left_join(tcp_anual) %>%
  mutate(
    precio_crudo_me_pesos_bbl = precio_crudo_me_usd_bbl * TCC,
    precio_crudo_mi_pesos_bbl = precio_crudo_mi_usd_bbl * TCC
  )


## Base regalias
precio_mi_regalias_crudo <- read_delim(here("data/secretaria_energia/regalias/precio_mercado_interno_crudo_regalias.csv"),
  ";",
  escape_double = FALSE, trim_ws = TRUE
) %>%
  rename(
  ...
  group_by(anio, unidad) %>%
  summarise(
    precio_mi_regalias_crudo = mean(total_tipo_crudo),
...
precio_mi_vs_internacional_idee_crudo <- read_excel(here("data/idee/Precios del petroleo crudo y derivados 1970 - 1989.xlsx"),
  skip = 1, sheet = 4
)
...
precio_mi_crudo <- precio_mi_regalias_crudo %>%
  ...
  full_join(precio_mi_mecon_crudo) %>%
  ...
  rename(
    regalias = precio_mi_regalias_crudo,
...
precio_mi_crudo <- precio_mi_crudo %>%
  ...
  select(
    anio, unidad, variable, precio_crudo_mdoint, everything(.),
...
vtas_precio_memo_ypf_gas <- read_excel(here("data/ypf/vtas_valor_cantidad_precio_gas.xlsx")) %>%
  ...
  mutate(
    precio_mi_ypf_gas_moneda_original = (valor_vendido / cant_vendidad) / 1000,
  ...
  ) %>% # aclaramos la unidad monetaria ya transformada
  select(
    anio, unidad, moneda, valor_vendido, cant_vendidad,
    precio_mi_ypf_gas, precio_mi_ypf_gas_moneda_original, tcc_dic, precio_mi_usd_ypf_gas
  )

# Anuario Combustible
anuario_de_combustible <- read_excel(here("data/anuario_de_combustibles/anuario de combustible.xlsx")) %>%
  ...
  mutate(
    precio_mi_anuario_gas = (precio_mi_anuario_gas / conversor_pesos$A) / 1000,
    unidad = "ars/Miles de m3"
  )
...
## Base regalias
# valores 93 - 98
precio_regalias_total_gas <- read_excel(here("data/secretaria_energia/regalias/precio_mi_gas.xlsx"),
  sheet = "jk"
) %>%
  filter(anio < 1999) %>%
  mutate(
    precio_total_gas = precio_mi_gas * 1000,
    unidad = "ars/Miles de m3"
  ) %>%
  select(-precio_mi_gas)

# valores post 99
precio_regalias_mi_gas <- read_delim(here("data/secretaria_energia/regalias/precio_mercado_interno_gas_regalias.csv"),
  ";",
  escape_double = FALSE, trim_ws = TRUE
) %>%
  rename(
  ...
  mutate(precio_mi_regalias_gas = case_when(
    anio < 1999 ~ precio_total_gas,
...
precio_mi_idee_gas <- read_excel(here("data/idee/Precios del gas natural y derivados 1970 - 1988.xlsx"),
  skip = 1, sheet = 2
) %>%
...
precio_mi_gas <- precio_regalias_mi_gas %>%
  ...
  mutate(
    variable = "Precio del mercado interno del gas natural según distintas fuentes",
    precio_gas_mdoint = case_when(
  ...
  rename(
    regalias = precio_mi_regalias_gas,
...
# Mecon MODIFICAR
precio_exportacion_crudo <- read_excel(here("data/mecon/precio-exportacion-crudo.xlsx"),
  skip = 2
)
...
# UN COMTRADE
# ojo q existe data_comtrade[data_comtrade$`Qty Unit`=="No Quantity",]
data_comtrade <- read_csv(here("data/un_comtrade/all_crude_gas_sict.csv")) %>%
  mutate(
    cantidad = case_when(
      Commodity == "Crude petroleum" ~
        conversor_kgbbl_q(`Netweight (kg)`),
      Commodity == "Gas, natural" ~
        conversor_kgm3_q(`Netweight (kg)`)
    ),
    unidad = case_when(
      Commodity == "Crude petroleum" ~
        "barriles",
    ...
    precio = `Trade Value (US$)` / cantidad
  ) %>%
  # rename(anio = Period,
  #        valor = "Trade Value (US$)") %>%
  select(
    anio = Period, Classification,
    trade_flow = "Trade Flow", reporter = Reporter, partner = Partner,
    commodity_code = "Commodity Code", commodity = Commodity,
    unidad, cantidad, valor = "Trade Value (US$)", precio
  )

expo_crudo_uncomtrade_hs <- read_csv(here("data/un_comtrade/expo_crudo_uncomtrade_hs.csv")) %>%
  mutate(
    unidad_precio = "USD/barriles",
    unidad_valor = "USD",
...
expo_crudo_sitc <- read_csv(here("data/un_comtrade/expo_crudo_sitc.csv")) %>%
  mutate(
    unidad_precio = "USD/barriles",
    unidad_valor = "USD",
    unidad_cantidad = "barriles"
...
brent_daily <- read_excel(here("data/eia/RBRTEd.xls"),
  sheet = "Data 1", skip = 2
) %>%
...
brent <- brent_a %>%
  ...
  select(anio, unidad, everything(.)) %>%
  rename(
    brent_iea = brent_spot,
    brent_historic = brent_nominal
  )

# WTI
wti_EIA_daily <- read_excel(here("data/eia/RWTCd.xls"),
  sheet = "Data 1", skip = 2
) %>%
...
wti_fred <- read_csv(here("data/fread/WTISPLC.csv")) %>%
...
wti_retencion_07 <- wti_EIA_daily %>%
  ...
  mutate(retencion = case_when(
    wti_eia >= 60.9 ~ (wti_eia - 42) / 42,
    wti_eia < 60.9 ~ 0.45
  )) %>%
  group_by(anio) %>%
  summarise(
    wti_eia = mean(wti_eia),
...
brent_rentencion_14 <- brent_daily %>%
  ...
  mutate(retencion = case_when(
    brent_spot >= 71 ~ (brent_spot - 70) / 70,
    brent_spot < 71 ~ 0.01
  )) %>%
  group_by(anio) %>%
  summarise(
...
precios_referencia_y_expo_crudo <- precio_expo_crudo %>%
  ...
  full_join(wti_fred) %>%
  mutate(
    precio_expo_crudo_indec = case_when(
      precio_expo_crudo_indec > 300 ~ NA_real_,
      T ~ precio_expo_crudo_indec
    ),
    precio_me_crudo = case_when(
      anio %in% 1993:2001 | anio %in% 2004:2014 ~ precio_expo_mecon_crudo,
...
expo_gas_comtrade <- read_csv(here("data/un_comtrade/expo_gas_sitc.csv")) %>%
  left_join(tcp_anual_b %>% select(anio, tcc)) %>%
  mutate(
    expo_precio_MMBTU = conversor_m3MMBTU_p(expo_precio_Mm3),
...
wellhead_price_eeuu <- read_excel(here("data/eia/natural_gas_wellhead_price_eeuu.xls"),
  sheet = "Data 1", skip = 2
) %>%
  rename(us_wellhead_price_ft3 = "U.S. Natural Gas Wellhead Price (Dollars per Thousand Cubic Feet)") %>%
  mutate(
    us_wellhead_price_Mm3 = conversor_ft3m3_p(us_wellhead_price_ft3 * 1000),
...
price_eeuu <- read_excel(here("data/eia/natural_gas_prices_usa.xls"),
  sheet = "Data 1", skip = 2
) %>%
  rename(
    us_wellhead_gas_price = `U.S. Natural Gas Wellhead Price (Dollars per Thousand Cubic Feet)`,
  ...
  select(
    us_wellhead_gas_price, us_import_gas_price, us_pipeline_import_price,
    us_lng_import_price, us_export_gas_price,
    us_export_gas_pipeline_price, us_export_lng_price
  ) %>%
  mutate(anio = seq(1922, length.out = nrow(.)))


price_eeuu_Mm3 <- price_eeuu %>%
  mutate_all(function(x) {
    conversor_ft3m3_p(1000)
  }) %>%
  mutate(
    anio = seq(1922, length.out = nrow(price_eeuu)),
    unidad = "USD/Miles de m3"
  )

price_eeuu_MMBTU <- price_eeuu %>%
  mutate_all(function(x) {
    conversor_ft3MMBTU_p(x / 1000)
  }) %>%
  mutate(
    anio = seq(1922, length.out = nrow(price_eeuu)),
    unidad = "USD/MMBTU"
...
# BP
bp_gas_prices <- read_excel(here("data/bp/bp-stats-review-2020-all-data.xlsx"),
...
# FMI
fmi_prices <- read_csv(here("data/fmi/PCPS_09-16-2020 15-07-10-66_timeSeries.csv"),
  col_types = cols(X38 = col_skip())
)

fmi_gas_price <- fmi_prices %>%
  filter(
    `Commodity Name` %in% c("Natural gas, EU", "Natural Gas, US Henry Hub Gas", "LNG, Asia"),
    `Unit Name` == "Index"
  ) %>%
  rename(
...
cuadro_4.3 <- read_excel(here("data/idee/Precios del gas natural y derivados 1970 - 1988.xlsx"),
  skip = 1, sheet = 3
) %>%
...
precio_idee_impo_gas <- cuadro_4.3 %>%
  ...
  left_join(ipc_promedio %>% select(anio, ipc_70)) %>%
  mutate(
    precio_gas_bolivia_idee = precio_gas_bolivia * ipc_70 / conversor_pesos$`$Ley`,
    unidad = "ars/Miles de m3"
  ) %>%
  left_join(tcp_anual_b %>% select(anio, tcc)) %>%
...
# a m3
precio_expo_bolivia_usd_ypfb <- read_excel(here("data/ypfb/precio_expo_bolivia.xlsx")) %>%
  mutate_at(
    .vars = c("precio_expo_bolivia_arg_gas", "precio_expo_bolivia_bzl_gas"),
    .funs = function(x) {
      conversor_ft3m3_p(x)
    }
  ) %>%
...
# a MM BTU
precio_expo_bolivia_usd_MMBTU_ypfb <- read_excel(here("data/ypfb/precio_expo_bolivia.xlsx")) %>%
  mutate_at(
    .vars = c("precio_expo_bolivia_arg_gas", "precio_expo_bolivia_bzl_gas"),
    .funs = function(x) {
      conversor_ft3MMBTU_p(x / 1000)
  ...
  mutate(unidad = "USD/MMBTU") %>%
  rename(
    precio_expo_bolivia_arg_ypfb = precio_expo_bolivia_arg_gas,
...
precio_expo_bolivia_pesos_ypfb <- precio_expo_bolivia_usd_ypfb %>%
  left_join(tcp_anual) %>%
  mutate(
    precio_expo_bolivia_arg_gas_pesos = precio_expo_bolivia_arg_gas * TCC,
...
gas_impo_bolivia_comtrade_usd <- read_csv(here("data/un_comtrade/gas_impo_bolivia_comtrade.csv")) %>%
  select(
    Period, "Trade Flow", Reporter, Partner, "Commodity Code", Commodity,
    Qty, trade_value_usd, precio_impo_gas_bolivia, unidad_precio, unidad_cantidad
  ) %>%
  rename(anio = Period) %>%
  mutate(
    precio_impo_gas_bolivia_MMBTU_comtrade = conversor_m3MMBTU_p(precio_impo_gas_bolivia),
    unidad = "USD/MMBTU"
  ) %>%
  filter(!anio %in% c(1994:1997))

# pasaje a pesos
gas_impo_bolivia_comtrade <- gas_impo_bolivia_comtrade_usd %>%
  left_join(tcp_anual_b, by = "anio") %>%
  mutate(
    precio_impo_gas_bolivia = precio_impo_gas_bolivia * tcc,
    unidad_precio = "ars/Miles de m3"
  )


# precio expo bolivia UN COMTRADE
gas_expo_bolivia_comtrade_usd <- read_csv(here("data/un_comtrade/gas_expo_bolivia_comtrade.csv")) %>%
  select(
    Period, "Trade Flow", Reporter, Partner, "Commodity Code", Commodity,
  ...
  rename(anio = Period) %>%
  mutate(
    precio_expo_gas_bolivia_arg_MMBTU_comtrade = conversor_m3MMBTU_p(precio_expo_gas_bolivia),
    unidad = "USD/MMBTU"
  )
...
precio_mdomundial_gas_MMBTU <- price_eeuu_MMBTU %>%
  ...
  mutate(
    ...
    precio_exportacion_gas_ar = case_when(
      anio < 1999 ~ precio_expo_gas_comtrade,
...
comparacion_bep <- precio_mdomundial_gas_bep %>%
  ...
  left_join(precios_referencia_y_expo_crudo %>%
    select(
      anio, brent_historic, wti_spot_price_fred,
...
importaciones_exportaciones_post2016 <- read_csv(here("data/secretaria_energia/sesco/importaciones-exportaciones-a-partir-del-2016-.csv"))

#### comercio exterior post 2010: "TD_comercio_exterior.xlsx"
comex_post2010 <- read_delim(here("data/secretaria_energia/sesco/comex_post2010.csv"),
  ";",
  escape_double = FALSE, trim_ws = TRUE,
  skip = 5
) %>%
...
expo_pre2010 <- read_delim(here("data/secretaria_energia/sesco/expo_pre2010.csv"),
  ";",
  escape_double = FALSE, trim_ws = TRUE
) %>%
  rename(
...
expo_usd_ton <- read_delim(here("data/secretaria_energia/sesco/expo_usd_ton.csv"),
  ";",
  escape_double = FALSE, trim_ws = TRUE,
  skip = 3
)
...
impo_pre2010 <- read_delim(here("data/secretaria_energia/sesco/impo_pre2010.csv"),
  ";",
  escape_double = FALSE, trim_ws = TRUE
) %>%
  rename(
...
comex_crudo_sesco <- comercio_exterior %>%
  filter(
    str_detect(producto, "PETROLEO") | str_detect(producto, "Cuenca") | str_detect(producto, "Crudo importado"),
    unidad != "Ton"
  ) %>%
  group_by(anio, variable, unidad) %>%
  ...
  mutate(
    producto = "crudo",
    valor = case_when(
      unidad == "m3" ~ conversor_m3bbl_q(valor),
      T ~ valor
    ),
    unidad = case_when(
      unidad == "m3" ~ "barriles",
...
# MECON
comex_hidrocarburos_mecon <- read_excel(here("data/mecon/base-mineria-e-hidrocarburos cuentas nacionales.xls"),
  ...
  mutate_all(as.double) %>%
  mutate(
    expo_bbl_crudo = conversor_m3bbl_q(expo_Mm3_crudo * 1000),
...
# INDEC - ComEx
# cantidades
cantidades_expo_hidro_indec <- read_csv(here("data/indec/cantidades_expo_hidro_indec.csv"),
  col_types = cols(X1 = col_skip())
) %>%
  rename(
    expo_indec_crudo = petroleo_crudo_expo,
    expo_indec_gas = gas_natural_expo
  )

# valor
expo_hidro_valor_indec <- read_csv(here("data/indec/expo_hidro_valor.csv"),
  col_types = cols(X1 = col_skip())
) %>%
  rename(
    expo_indec_crudo = petroleo_crudo_expo,
...
expo_q_crudo <- comex_crudo_sesco %>%
  ...
  mutate(
    expo_crudo = case_when(
      anio < 1999 ~ expo_comtrade_crudo,
...
expo_usd_crudo <- comex_crudo_sesco %>%
  ...
  mutate(
    expo_crudo_mecon = expo_crudo_mecon * 1e6,
    expo_crudo = case_when(
      anio < 1994 ~ expo_comtrade_crudo,
...
impo_crudo <- comex_crudo_sesco %>%
  ...
  mutate(
    fuente = "Sec. Energía - SESCO",
    variable = "Importación de petróleo crudo"
  ) %>%
  bind_rows(data_comtrade %>%
    filter(
      trade_flow == "Import",
      commodity_code == 33101
    ) %>%
    mutate(
      producto = "crudo",
      variable = "Importación de petróleo crudo", fuente = "UN-Comtrade"
    ) %>%
    select(anio, variable, producto,
      USD = valor, barriles = cantidad, fuente
    ) %>%
    pivot_longer(
      cols = c(USD, barriles), names_to = "unidad",
      values_to = "valor", names_repair = "unique"
    )) %>%
  bind_rows(comex_hidrocarburos_mecon %>%
    select(anio, valor = impo_bbl_crudo) %>%
    mutate(
      fuente = "MECON",
...
comex_gas <- comercio_exterior %>%
  filter(
    str_detect(producto, "GAS NATURAL") | str_detect(producto, "Gas Natural") |
...
expo_sesco_q_gas <- comex_gas %>%
  ...
  mutate(
    expo_sesco_gas = replace_na(expo_sesco_gas, 0),
...
# expo UN Comtrade
expo_gas_sitc <- read_csv(here("data/un_comtrade/expo_gas_sitc.csv")) %>%
...
expo_q_gas <- expo_sesco_q_gas %>%
  ...
  mutate(expo_gas = case_when(
    anio < 1997 ~ expo_gas_comtrade,
...
impo_gas <- comex_gas %>%
  ...
  mutate(
    fuente = "Sec. Energía - SESCO",
    variable = "Importación de gas natural"
  ) %>%
  bind_rows(data_comtrade %>%
    filter(
      trade_flow == "Import",
    ...
    select(anio, variable, producto,
      USD = valor, m3 = cantidad, fuente
    ) %>%
    pivot_longer(
      cols = c(USD, m3), names_to = "unidad",
      values_to = "valor", names_repair = "unique"
    )) %>%
  bind_rows(comex_hidrocarburos_mecon %>%
    select(anio, valor = impo_MMm3_gas) %>%
    mutate(
...
retenciones_campodonico <- read_excel(here("data/afip/retenciones.xlsx"),
  ...
  left_join(tcp_anual %>% select(-TCP)) %>%
  mutate(
    retenciones_hc = retenciones_hg * TCC * 1000000,
    unidad = "ars"
  ) %>%
  select(anio, unidad, retenciones_hc)

retenciones_crudo <- read_excel(here("data/afip/retenciones.xlsx")) %>%
  rename(
    retenciones_crudo_jk = retenciones_jk,
    retenciones_crudo_bfr = "retenciones_bfr_M$2008"
  ) %>%
  mutate(
    retenciones_crudo_bfr = retenciones_crudo_bfr * ipc08_bfr * 1000,
    unidad = "ars"
  ) %>%
  select(-ipc08_bfr) %>%
...
lista_regalias <- list()
for (i in productos) {
  regalias_i <- read_delim(paste0(here("data/secretaria_energia/regalias/regalias_"), i, ".csv"),
    ";",
    escape_double = FALSE, trim_ws = TRUE,
    skip = 1
  )
...
regalias_sec_en <- regalias_crudo %>%
  ...
  left_join(tcp_anual %>% select(-TCP)) %>%
  mutate(
    regalias_se = case_when(
      moneda == "USD" ~ regalias_se * TCC,
      T ~ regalias_se
    ),
    unidad = "ars"
  ) %>%
...
# FR y DB
regalias_fr <- read_excel(here("data/ypf/Anuario Estadistico YPF - Datos ventas 1963-1991.xlsx"),
...
regalias <- regalias_sec_en %>%
  ...
  mutate(regalias_total = case_when(
    anio < 1991 ~ 0, # regalias_fr,
...
subsidios_cefip <- read_excel(here("data/cefip/subsidios.xlsx")) %>%
  ...
  left_join(ganancia_y_pbi %>%
    select(anio, pbi)) %>%
  mutate(
    subsidios_porcentaje_pbi = as.double(subsidios_porcentaje_pbi),
...
# ejes
subsidios_ejes <- read_excel(here("data/ejes/subsidios.xlsx")) %>%
...
cepal_1991 <- read_excel(here("data/cepal/cepal_1991.xlsx")) %>%
  mutate(
    valor = valor / conversor_pesos$A / 10^3,
...
empleo <- read_excel(here("data/mecon/base-mineria-e-hidrocarburos cuentas nacionales.xls"),
...
salarios <- read_excel(here("data/mecon/base-mineria-e-hidrocarburos cuentas nacionales.xls"),
  sheet = "Remuneraciones", col_types = c(
    "numeric",
  ...
  mutate(
    unidad_salario = "Pesos corrientes",
...
masa_salarial_ccnn <- empleo %>%
  ...
  mutate(
    unidad_masa_salarial = "Millones de pesos corrientes",
...
remuneraciones_anual <- read_excel(here("data/min_trabajo/nacional_serie_remuneraciones_anual.xlsx"),
  sheet = "C 5", col_types = c(
    "skip",
...
empleo_anual <- read_excel(here("data/min_trabajo/nacional_serie_empleo_anual.xlsx"),
  sheet = "C 5", col_types = c(
    "skip",
    "text", "text", "text", "text", "text",
    "text", "text", "text", "text", "text",
...
# Extender remuneraciones con datos mensuales 2020-YEAR_LAST
remuneraciones_nueva <- read_excel(
  here("data/min_trabajo/nacional_serie_remuneraciones_mensual_7.xlsx"),
  sheet = "C 8", skip = 4
) %>%
  rename(rama_actividad = "Rama de actividad") %>%
  select(-1, -3) %>%
  filter(str_detect(rama_actividad, "petróleo")) %>%
  pivot_longer(-rama_actividad, names_to = "fecha_str", values_to = "valor") %>%
  mutate(
    anio = coalesce(
      as.character(year(suppressWarnings(parse_date_time(fecha_str, orders = c("Ymd HMS", "Ymd"))))),
      str_extract(fecha_str, "\\d{4}")
    ),
    valor = suppressWarnings(as.double(valor))
  ) %>%
  filter(as.integer(anio) > YEAR_EMPLEO_CORTE, !is.na(valor)) %>%
  group_by(rama_actividad, anio) %>%
  summarise(valor = mean(valor, na.rm = TRUE), .groups = "drop") %>%
  mutate(
    unidad_salario = "Pesos corrientes",
    rama_actividad = case_when(
      str_detect(rama_actividad, "Actividades.*servicios") ~ "remuneracion_servicios_extraccion",
      str_detect(rama_actividad, "Extracción de petróleo") ~ "remuneracion_extraccion_petroleo_gas"
    )
  ) %>%
  filter(!is.na(rama_actividad)) %>%
  pivot_wider(names_from = rama_actividad, values_from = valor)

remuneraciones_anual <- bind_rows(remuneraciones_anual, remuneraciones_nueva)

# Extender empleo con datos trimestrales 2020-YEAR_LAST
empleo_nueva <- read_excel(
  here("data/min_trabajo/nacional_serie_empleo_trimestral_6.xlsx"),
  sheet = "C5", skip = 2
) %>%
  rename(rama_actividad = "Sector") %>%
  filter(str_detect(rama_actividad, "petróleo")) %>%
  pivot_longer(-rama_actividad, names_to = "trimestre", values_to = "valor") %>%
  filter(str_detect(trimestre, "^[1-4]º Trim\\s+\\d{4}$")) %>%
  mutate(
    anio = as.character(str_extract(trimestre, "\\d{4}")),
    valor = suppressWarnings(as.double(valor))
  ) %>%
  filter(!is.na(anio), as.integer(anio) > YEAR_EMPLEO_CORTE, !is.na(valor)) %>%
  group_by(rama_actividad, anio) %>%
  summarise(valor = mean(valor, na.rm = TRUE), .groups = "drop") %>%
  mutate(
    unidad_empleo = "Puestos de trabajo",
    rama_actividad = case_when(
      str_detect(rama_actividad, "Actividades.*servicios") ~ "empleo_servicios_extraccion",
      str_detect(rama_actividad, "Extracción de petróleo") ~ "empleo_extraccion_petroleo_gas"
    )
  ) %>%
  filter(!is.na(rama_actividad)) %>%
  pivot_wider(names_from = rama_actividad, values_from = valor)

empleo_anual <- bind_rows(empleo_anual, empleo_nueva)
...
masa_salarial_hidrocarburos <- masa_salarial_ccnn %>%
  select(anio,
    unidad = unidad_masa_salarial, masa_salarial_extraccion_ccnn = masa_salarial_extraccion,
    masa_salarial_total_ccnn = masa_salarial_total
  ) %>%
  full_join(masa_salarial_oede %>%
    select(anio,
      unidad = unidad_masa_salarial,
...
# empleo, remuneracion y masa salarial to export
empleo_y_remuneraciones <- masa_salarial_ccnn %>%
  select(!contains("unidad")) %>%
  reshape2::melt(id.vars = c("anio"), value.name = "valor", variable.name = "var") %>%
  mutate(fuente = "CCNN") %>%
  rbind(
    masa_salarial_oede %>%
      select(!contains("unidad")) %>%
      reshape2::melt(id.vars = c("anio"), value.name = "valor", variable.name = "var") %>%
      mutate(fuente = "Ministerio de Trabajo - OEDE")
  ) %>%
  mutate(
    variable = case_when(
      str_detect(var, "empleo") ~ "Empleo",
      str_detect(var, "masa_salarial") ~ "Masa salarial",
      str_detect(var, "remuneracion|salario") ~ "Salario",
      T ~ NA_character_
    ),
    unidad = case_when(
      str_detect(var, "empleo") ~ "Puestos de trabajo",
      str_detect(var, "masa_salarial") ~ "Pesos corrientes",
      str_detect(var, "remuneracion|salario") ~ "Pesos corrientes",
      T ~ NA_character_
    ),
    sector = case_when(
      str_detect(var, "servicios") ~ "Servicios de apoyo a la extracción",
      str_detect(var, "extraccion") ~ "Extracción de petróleo y gas",
      str_detect(var, "total") ~ "Total"
    ),
    codigo_variable = case_when(
      str_detect(var, "empleo") ~ "Ocup",
      str_detect(var, "masa_salarial") ~ "Kvc",
      str_detect(var, "remuneracion|salario") ~ "Rem",
      T ~ NA_character_
    ),
    valor = case_when(
      variable == "Masa salarial" ~ valor * 1e6,
      T ~ valor
    ),
...
ypf_seg <- read_delim(here("data/ypf/ypf_segmentos.csv"),
  ...
  mutate(
    sector = case_when(
      sector == "quimica" ~ "petroquimica",
      T ~ sector
    ),
    anio = year(fecha)
  )

petrobras_ar_seg <- read_delim(here("data/balances/petrobras_arg_segmentos.csv"),
  ",",
  escape_double = FALSE, trim_ws = TRUE
) %>%
  select(-"...1")

stock_segmentos <- ypf_seg %>%
...
union_segmentos <- stock_segmentos %>%
  left_join(ipc_promedio %>% select(anio, ipc_18)) %>%
  mutate(
    valor = valor / ipc_18,
    unidad = "Millones de pesos 2018"
  ...
  ungroup() %>%
  mutate(
    fuente = "Activo de YPF y Petrobras",
    variable_sector = paste(variable, sector, sep = " ")
  )


## Stock por empresas
stock_balances_empresas <- read_csv(here("data/balances/balances_arg.csv"),
  ...
  mutate(
    valor = valor / ipc_18,
    empresa = case_when(
      empresa == "petrobras_ar" ~ "Petrobras",
...
# AFIP
tasa_depreciacion_afip <- read_excel(here("data/afip/gcia_v8.xlsx"),
  skip = 4
) %>%
  select(-c(49:ncol(.))) %>%
  group_by(rama, year) %>%
...
# Balances
read_csv(here("data/balances/balances_arg.csv"),
  col_types = cols(X1 = col_skip())
) %>%
  filter(empresa == "YPF") %>%
  group_by(empresa) %>%
...
datos_afip <- read_excel(here("data/afip/gcia_v8.xlsx"),
  skip = 4
) %>%
  select(-c(49:ncol(.))) %>%
  rename(
  ...
  mutate(
    unidad = "pesos corrientes",
    sector = case_when(
      rama == "petro" ~ "extraccion_petroleo_gas",
...
stock_afip <- datos_afip %>%
  ...
  summarise(
    KTA = disponibilidades + bscambio + inventarios + bsuso,
...
##################################
# Memorias YPF
balance_ypf <- read_excel(here("data/ypf/Calculos Betania Tg.xls"),
...
metros_perforados_anterior_al_2009 <- read_csv(here("data/secretaria_energia/sesco/metros-perforados-anterior-al-2009.csv")) %>%
  rename(cantidad = Cantidad)
...
listado_pozos <- read_csv(here("data/secretaria_energia/cap_iv/listado-de-pozos-cargados-por-empresas-operadoras.csv")) %>%
...
ppye_ypf <- ypf_seg %>%
  ...
  mutate(
    activo_upstream = variacion_interanual(activo_upstream),
...
stock_y_pozos_ypf <- ppye_ypf %>%
  mutate(indice_pozos_06 = generar_indice(
    serie = pozos,
    fecha = anio,
    fecha_base = 2006
  )) %>%
  mutate(
    ppye_ypf_estimado = anio_base_ypf_bolsar * indice_pozos_06,
...
estimacion_ppye_total_via_ypf <- stock_y_pozos_ypf %>%
  ...
  mutate(
    valor = valor / ypf_sobre_total,
    estimacion = case_when(
      estimacion == "ppye_ypf_estimado" ~ "ppye_estimado_total",
...
mip_97 <- read_excel(here("data/mip/mip_matriz12.xls"),
  skip = 7
) %>%
  select(-"...1") %>%
  rename(actividad = "...2")
...
COU <- read_excel(here("data/mip/sh_cou_06_16.xls"),
  skip = 2
) %>%
  rename(
    producto = "...1",
...
consumo_k_fijo_ypf <- (read_csv(here("data/balances/balances_arg.csv"),
  col_types = cols(X1 = col_skip())
) %>%
  filter(empresa == "YPF") %>%
  group_by(empresa) %>%
...
var((read_csv(here("data/balances/balances_arg.csv"),
  col_types = cols(X1 = col_skip())
) %>%
  filter(empresa == "YPF") %>%
  group_by(fecha) %>%
...
ccnn_oficial <- read_excel(here("data/mecon/base-mineria-e-hidrocarburos cuentas nacionales.xls"), sheet = "Cuentas Nacionales", skip = 5) %>%
  ...
  left_join(masa_salarial_hidrocarburos %>%
    select(anio, unidad,
      ...
      ms_extr = masa_salarial_extraccion_oede
    ), by = c("anio", "unidad")) %>%
  mutate(
    ebe_tot = va_extraccion_y_servicios_hidrocarburos - ms_tot,
    ebe_extr = va_extraccion_hidrocarburos - ms_extr,
    fuente = "CCNN oficial"
  ) %>%
  rename(
    vbp_extr = vbp_extraccion_hidrocarburos,
...
mean_coef_ms <- ccnn_oficial %>%
  summarise(
    coef_ms_tot = mean(ms_tot / vbp_tot, na.rm = T),
    coef_ms_extr = mean(ms_extr / vbp_extr, na.rm = T)
  )

median_coef_ms <- ccnn_oficial %>%
  summarise(
    coef_ms_tot = median(ms_tot / vbp_tot, na.rm = T),
...
# Base INDEC
x <- read_excel(here("data/indec/sh_VBP_VAB_03_21.xls"), sheet = 3, skip = 3) %>%
  na.omit()
names(x) <- c("sector", 2004:2020)
vbp_ccnn <- x %>%
  gather(key = anio, value = vbp_tot, 2:ncol(.))

y <- read_excel(here("data/indec/sh_VBP_VAB_03_21.xls"), sheet = 5, skip = 4) %>%
...
## VA y VBP precios 93
x <- read_excel(here("data/mecon/PBI_Base 1993_PM.xlsx"),
  sheet = "VBP c", col_types = c(
    "text",
    "numeric", "numeric", "numeric",
    "numeric", "numeric", "numeric",
    "numeric", "numeric", "numeric",
...
y <- read_excel(here("data/mecon/PBI_Base 1993_PM.xlsx"),
  sheet = "VAB c", skip = 6
) %>%
  select(1, !contains("I"))
...
coef_consumo_intermedio <- coef_consumo_intermedio %>%
  mutate(coef_ci = case_when(
    anio <= 2004 ~ coef_tec_93_index * ci_base,
    T ~ coef_ci
  )) %>%
...
# writexl::write_xlsx(precios_y_cantidades, path = "results/argentina/insumos_criterio_ccnn.xlsx")

## Criterio CCNN
criterio_ccnn <- precios_y_cantidades %>%
  left_join(coef_consumo_intermedio, by = "anio") %>%
  mutate(
    crudo_mdo_interno = prod_crudo - expo_crudo,
...
empalme_ccnn <- ccnn_oficial %>%
  ...
  mutate(
    vbp_extr = case_when(
      anio < 2004 ~ ccnn_oficial$vbp_extr[ccnn_oficial$anio == 2004] * vbp_criterio_ccnn_04,
      T ~ vbp_extr
    ),
    vbp_tot = case_when(
      anio < 2004 ~ ccnn_oficial$vbp_tot[ccnn_oficial$anio == 2004] * vbp_criterio_ccnn_04,
      T ~ vbp_tot
    ),
    ci_extr = vbp_extr * coef_ci,
    ci_tot = vbp_tot * coef_ci,
    va_tot = case_when(
      is.na(va_tot) ~ vbp_tot - ci_tot,
      T ~ va_tot
    ),
    va_extr = case_when(
      is.na(va_extr) ~ vbp_extr - ci_extr,
      T ~ va_extr
    ),
    ms_tot = case_when(
      is.na(ms_tot) ~ vbp_tot * mean_coef_ms$coef_ms_tot,
      T ~ ms_tot
    ),
    ms_extr = case_when(
      is.na(ms_extr) ~ vbp_tot * mean_coef_ms$coef_ms_extr,
      T ~ ms_extr
    ),
    ebe_tot = case_when(
      is.na(ebe_tot) ~ va_tot - ms_tot,
      T ~ ebe_tot
    ),
    ebe_extr = case_when(
      is.na(ebe_extr) ~ va_extr - ms_extr,
      T ~ ebe_extr
    ),
    fuente = "Empalme CCNN",
    unidad = "Millones de pesos corrientes"
...
valor_total_produccion_corr <- ccnn_oficial %>%
  ...
  gather(key = variable, value = valor, 4:13)
...
petrobras_arg_segmentos <- read_csv(here("data/balances/petrobras_arg_segmentos.csv"),
  col_types = cols(X1 = col_skip())
)
...
tasa_ganancia_rama <- empalme_ccnn %>%
  ...
  left_join(
    stock_estimado %>%
      ...
      mutate(
        fuente_pozo = "Estimación sin pozos",
        valor = valor * ipc_18
      ) %>%
...
renta_dif_precios_crudo <- prod_merge_crudo %>%
  ...
  left_join(
    ...
    by = c("anio")
  ) %>% # uno con precio interno
  left_join(
    precios_referencia_y_expo_crudo %>%
      mutate(brent = case_when(
        anio < 1987 ~ brent_historic,
        anio >= 1987 ~ brent_iea
      )) %>%
      select(anio, precio_me_crudo, brent),
    by = c("anio")
  ) %>% # uno con precio externo
  left_join(tcp_anual_b, by = "anio") %>%
  mutate(
    renta_dif_precios_crudo = prod_mdo_interno * precio_me_crudo * tcp - prod_mdo_interno * precio_crudo_mdoint * tcc,
  ...
  select(anio,
    unidad_cantidad = unidad, prod_crudo, expo_crudo, existencias_crudo, prod_mdo_interno,
...
renta_diferencial_precios_gas <- prod_merge_gas_MMBTU %>%
  ...
  select(anio,
    unidad_cantidad = unidad, prod_gas, expo_gas, prod_mdo_interno,
...
renta_tcp_crudo <- renta_dif_precios_crudo %>%
  ...
  mutate(
    renta_sobrevaluacion_crudo = expo_crudo * precio_externo_crudo * tcp - expo_crudo * precio_externo_crudo * tcc,
...
renta_directo <- renta_tg %>%
  ...
  left_join(subsidios_hidrocarburos %>%
    ...
    mutate(
      subsidios = (subsidios_cefip / 10^6) / ipc_18,
  ...
  left_join(ipc_promedio %>% select(anio, ipc_18), by = "anio") %>%
  mutate(
    renta_con_tg_normal = (ppye / ipc_18) * (tg_hidrocarburos - tg_normal),
...
renta_indirecto <- renta_dif_precios_crudo %>%
  ...
  mutate_all(function(x) {
    x / 10^6
  }) %>% # las vbles cargadas anteriormente se pasan a MM
  ...
  mutate(
    ...
    renta_total = rowSums(cbind(
      renta_dif_precios_gas, renta_dif_precios_crudo,
      renta_sobrevaluacion_crudo, renta_sobrevaluacion_gas,
      renta_empresas, regalias_total, retenciones, subsidios * -1
    ), na.rm = T),
  ...
  rename(
    renta_diferencial_precios_gas = renta_dif_precios_gas,
...
costos <- produccion_total %>%
  ...
  left_join(valor_total_produccion %>%
    ...
    filter(
      variable %in% c("ci_extr", "ms_extr"),
      fuente %in% c("Empalme CCNN")
    ) %>% # "CCNN oficial", "Criterio CCNN"
    mutate(
      valor = (valor * 10^6) * ipc_18 / tcc,
      unidad = "USD tcc"
    ) %>%
    spread(key = variable, value = valor), by = "anio") %>%
  left_join(renta_directo %>%
    select(
      anio, # stock_seleccionado,
...
renta_comparacion <- renta_indirecto %>%
  select(anio, tcc, tcp, ipc_18, retenciones, regalias_total, contains("renta"), subsidios) %>%
  # select( -(13:ncol(.))) %>%
  select(-c(renta_pv, renta_pbi, renta_usd_tcc, renta_usd_tcp)) %>%
  filter(anio > 1962) %>%
  mutate(
    retenciones = retenciones * ipc_18 / tcc,
    regalias_total = regalias_total * ipc_18 / tcc,
    renta_diferencial_precios_crudo = renta_diferencial_precios_crudo * ipc_18 / tcc,
    renta_diferencial_precios_gas = renta_diferencial_precios_gas * ipc_18 / tcc,
    renta_expo_sobrevaluada_crudo = renta_expo_sobrevaluada_crudo * ipc_18 / tcc,
    renta_expo_sobrevaluada_gas = renta_expo_sobrevaluada_gas * ipc_18 / tcc,
    ...
    renta_total = (renta_total * ipc_18) / tcc,
    unidad = "Millones de USD",
    autor = "Propia"
  ) %>%
  rename(
  ...
  select(
    anio, unidad, autor, everything(.),
    -c(
      ipc_18, tcc, tcp, renta_expo_sobrevaluada_crudo, renta_expo_sobrevaluada_gas,
      renta_diferencial_precios_gas, renta_diferencial_precios_crudo
    )
  ) %>%
  gather(
...
# expo q crudo
expo_q_crudo_dt <- as.data.table(expo_q_crudo)
expo_q_crudo_dt[, `:=`("expo_crudo" = NULL)]
expo_q_crudo_dt[, variable := "Exportación de petróleo crudo"]
setnames(expo_q_crudo_dt,
...
expo_q_crudo_dt <- melt(expo_q_crudo_dt,
  id.vars = c("anio", "variable", "unidad"),
  variable.name = "fuente", value.name = "valor"
)
# expo_q_crudo = expo_q_crudo[!is.na(valor)]
expo_q_crudo_dt[, codigo_variable := fcase(
  fuente == "Sec. Energía - SESCO", "X_crudo_sesco",
  fuente == "MECON", "X_crudo_mecon",
  fuente == "INDEC", "X_crudo_indec",
  fuente == "UN-Comtrade", "X_crudo_comtrade"
)]
# expo USD crudo
expo_usd_crudo <- as.data.table(expo_usd_crudo)
expo_usd_crudo[, expo_crudo := NULL]
expo_usd_crudo[, variable := "Exportación de petróleo crudo"]
setnames(expo_usd_crudo,
  old = c("expo_crudo_sesco", "expo_crudo_mecon", "expo_indec_crudo", "expo_comtrade_crudo"),
  new = c("Sec. Energía - SESCO", "MECON", "INDEC", "UN-Comtrade")
)
expo_usd_crudo <- melt(expo_usd_crudo,
  id.vars = c("anio", "variable", "unidad"),
  variable.name = "fuente", value.name = "valor"
)
# expo_usd_crudo = na.omit(expo_usd_crudo)
expo_usd_crudo[, codigo_variable := fcase(
  fuente == "Sec. Energía - SESCO", "X_crudo_sesco",
  fuente == "MECON", "X_crudo_mecon",
  fuente == "INDEC", "X_crudo_indec",
  fuente == "UN-Comtrade", "X_crudo_comtrade"
...
# impo crudo
impo_crudo_dt <- as.data.table(impo_crudo)
impo_crudo_dt[, producto := NULL]
impo_crudo_dt[, codigo_variable := fcase(
  fuente == "Sec. Energía - SESCO", "M_crudo_sesco",
  fuente == "UN-Comtrade", "M_crudo_comtrade",
  fuente == "MECON", "M_crudo_mecon"
)]
...
expo_q_gas_MMBTU[, `:=`("expo_gas" = NULL)]
expo_q_gas_MMBTU[, variable := "Exportación de gas natural"]
...
expo_q_gas_MMBTU[, codigo_variable := fcase(
  fuente == "Sec. Energía - SESCO", "X_gas_sesco",
  fuente == "MECON", "X_gas_mecon",
  fuente == "INDEC", "X_gas_indec",
  fuente == "UN-Comtrade", "X_gas_comtrade"
)]
expo_q_gas_MMBTU <- expo_q_gas_MMBTU[fuente != "INDEC", ]

# expo USD gas
expo_usd_gas <- as.data.table(expo_usd_gas)
expo_usd_gas[, expo_gas := NULL]
expo_usd_gas[, variable := "Exportación de gas natural"]
setnames(expo_usd_gas,
  old = c("expo_sesco_gas", "expo_gas_mecon", "expo_gas_indec", "expo_gas_comtrade"),
  new = c("Sec. Energía - SESCO", "MECON", "INDEC", "UN-Comtrade")
)
expo_usd_gas <- melt(expo_usd_gas,
  id.vars = c("anio", "variable", "unidad"),
  variable.name = "fuente", value.name = "valor"
)
# expo_usd_gas = expo_usd_gas[!is.na(valor)]
expo_usd_gas[, codigo_variable := fcase(
  fuente == "Sec. Energía - SESCO", "X_gas_sesco",
  fuente == "MECON", "X_gas_mecon",
  fuente == "INDEC", "X_gas_indec",
  fuente == "UN-Comtrade", "X_gas_comtrade"
...
# impo_gas
impo_gas_dt <- as.data.table(impo_gas)
impo_gas_dt[, producto := NULL]
impo_gas_dt[, codigo_variable := fcase(
  fuente == "Sec. Energía - SESCO", "M_gas_sesco",
  fuente == "UN-Comtrade", "M_gas_comtrade"
)]
impo_gas_dt[, valor := fcase(
  unidad == "m3", conversor_m3MMBTU_q(valor),
  unidad == "USD", valor
)]
impo_gas_dt[, unidad := fcase(
  unidad == "m3", "MMBTU",
  unidad == "USD", "USD"
)]
# impo_gas[, valor := fcase(unidad == "m3", valor/Hmisc::yearDays(as.Date(paste0(anio, "-01-01" ))) ,
#                           # default = valor)]
#                           unidad == "USD", valor)]
...
precio_mi_crudo[, codigo_variable := fcase(
  fuente == "Sec. Energía - Regalías", "Pi_crudo_regalias",
...
setnames(precio_interno_gas_mmbtu_usd,
  old = c(
    "anuario", "regalias", "mecon",
    # "mecon_ccnn",
    "ypf", "idee"
  ),
  new = c(
    "Anuario de combustibles", "Sec. Energía - Regalías", "MECON",
    # "MECON/CCNN",
    "YPF", "IDEE"
  )
)
...
precio_interno_gas_mmbtu_usd[, codigo_variable := fcase(
  fuente == "Sec. Energía - Regalías", "Pi_gas_regalias",
...
precios_referencia_crudo <- precios_referencia_y_expo_crudo[, .(
  anio, unidad,
...
precios_referencia_crudo[, codigo_variable := fcase(
  fuente == "Brent (Inflation Data)", "Pext_brent",
...
precios_exportacion_crudo <- precios_referencia_y_expo_crudo[, .(
  anio, unidad,
...
setnames(
  precios_exportacion_crudo,
  c(
    "precio_expo_regalias_crudo", "precio_expo_mecon_crudo", "precio_expo_crudo_indec",
...
precios_exportacion_crudo[, codigo_variable := fcase(
  fuente == "Sec. Energía - Regalías", "Px_crudo_regalias",
...
precios_referencia_gas <- precio_mdomundial_gas_MMBTU[, c(
  1:21,
  grep(
    "precio_expo_bolivia_arg_ypfb",
    names(precio_mdomundial_gas_MMBTU)
  ),
  grep(
    "precio_expo_bolivia_bzl_ypfb",
    names(precio_mdomundial_gas_MMBTU)
  ),
  grep(
...
setnames(
  precios_referencia_gas,
  c(
    "us_wellhead_gas_price", "us_import_gas_price", "us_pipeline_import_price", "us_lng_import_price",
  ...
  c(
    "Pext_gas_us_wellhead", "Pext_gas_us_import", "Pext_gas_us_pipeline_import", "Pext_lng_us_import",
...
setnames(
  precios_exportacion_gas,
  c(
    "precio_expo_gas_comtrade", "precio_expo_gas_regalias", "precio_expo_gas_indec",
...
renta_indirecto_dt <- renta_indirecto_dt[anio > 1961, c(
  "anio", "renta_diferencial_precios_crudo",
...
renta_indirecto_dt[, c(
  "Rdifp_crudo", "Rdifp_gas", "Rret", "Rreg",
...
renta_directo[, c("Kcfa", "TG_pyg", "TGnorm", "PV", "Subs", "Gnorm_pyg", "Rtt") := lapply(
  .SD,
...
#
crudo_sobrevaluacion <- as.data.table(renta_tcp_crudo[, c("anio", "renta_sobrevaluacion_crudo")])
setnames(crudo_sobrevaluacion, "renta_sobrevaluacion_crudo", "Rsvx_crudo")
...
setnames(
  crudo_dif_precios, c(
    "prod_crudo", "expo_crudo", "existencias_crudo",
...
#
gas_sobrevaluacion <- as.data.table(renta_tcp_gas[, c(c("anio", "renta_sobrevaluacion_gas"))])
...
retenciones_pg <- melt(retenciones_pg,
  id.vars = c("anio", "unidad"),
...
retenciones_pg[, fuente := fcase(
  var == "retenciones_crudo_jk", "AFIP",
...
regalias_to_vars <- melt(regalias_pg,
  id.vars = c("anio", "unidad"),
  variable.name = "codigo_variable", value.name = "valor"
)
...
subsidios_pg <- melt(subsidios_pg,
  id.vars = c("anio", "unidad"),
  variable.name = "fuente", value.name = "valor"
)
...
setnames(
  tg_pg_total, c("ppye", "tg_hidrocarburos", "union_tg", "renta_con_tg_union"),
...
tg_pg_empresas[, c(
  "fecha", "empresa", "sector", "moneda", "KTA", "gcia_ant",
  "tg_ant", "tg_desp"
)]

div_millon <- function(x) {
  x / 10^6
}
...
setnames(
  ccnn_pg, c(
    "vbp_tot", "va_tot", "ci_tot", "ms_tot", "ebe_tot",
    "vbp_extr", "va_extr", "ci_extr", "ms_extr", "ebe_extr"
  ),
  c(
    "VBP_pyg", "VA_pyg", "CI_pyg", "Kvc_pyg", "G_pyg",
...
setnames(
  VBPextTcp,
  c(
    "vbp_tot", # "vbp_extr",# "coef_ci",
    "ci_tot", # "ci_extr",
    "va_tot", # "va_extr", "ms_extr" ,
    "ms_tot",
    "ebe_tot" # "ebe_extr"  ),
  ), c(
    "VBP_gyp", # "VBP_extr",# "coef_CI",
    "CI_gyp", # "CI_extr",
...
#
costos_comparacion <- readxl::read_excel(here("results/argentina/costos.xlsx"))
...
setnames(
  costos_pg_ccnn, c(
    "produccion_total", "costos_totales",
...
all_raw_data <- rbindlist(list(
  ...
  expo_q_crudo_dt, expo_usd_crudo,
  expo_q_gas_MMBTU, expo_usd_gas,
  impo_crudo_dt, impo_gas_dt,
  ...
  regalias_to_vars, retenciones_pg, subsidios_pg,
  empleo_y_remuneraciones
), use.names = TRUE)
write.csv(all_raw_data, here("results/argentina/variables.csv"),
  row.names = F,
  fileEncoding = "UTF-8"
)
...
# capture.output(coeficientes, file = here("results/argentina/coeficientes.txt"))

indice <- read_excel(here("results/argentina/metadata_arg.xlsx"), sheet = "indice")
variables <- read_excel(here("results/argentina/metadata_arg.xlsx"), sheet = "variables")

write.csv(ipc_promedio, here("data/indices/ipc_annual_avg.csv"), row.names = F)
...
writexl::write_xlsx(
  list(
    ...
    segmentos_YPF_PetBR = stock_segmentos,
    ccnn_pg = ccnn_pg,
  ...
  path = here("results/argentina/renta_de_la_tierra_hidrocarburifera_arg.xlsx")
)

write.csv(stock_segmentos, here("results/argentina/base_csv/stock_segmentos.csv"), row.names = F)
write.csv(stock_balances_empresas, here("results/argentina/base_csv/stock_balances_empresas.csv"), row.names = F)
write.csv(stock_ypf, here("results/argentina/base_csv/stock_ypf.csv"), row.names = F)
