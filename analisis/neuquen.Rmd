...
produccion_provincia <- read_excel("C:/Archivos/repos/hidrocarburos/analisis/data/produccion_provincia.xlsx", 
  ...
  select(fecha, mes, anio, unidad, everything(.)) %>% 
  group_by(fecha, mes, anio, unidad) %>% 
...
writexl::write_xlsx(x = list(produccion_provincia, precio_provincia, precio_tipo_crudo),
                      path = "resultados/neuquen/produccion_y_precios_x_provincia(Regalias).xlsx")
