...
ganancia_y_pbi <- read_excel("datasets/ganancia y pbi.xlsx") %>% 
...
# **Estimación JK**
renta_hidrocarburos_jk <- read_excel("renta_hidrocarburos_jk.xls", 
    skip = 2) %>% 
...
renta_hidrocarburos_fr <- read_excel("datasets/renta_hidrocarburos_FR.xlsx") %>%
  select(-unidad) %>% 
...
prod_merge_crudo <-  prod_regalias_crudo %>% 
  ...
  mutate_at(.vars = c(   "crudo_regalias" ,"crudo_mecon",   
                       "crudo_sesco",    "crudo_anuario" ), 
            .funs = conversor_m3bbl_q) %>%  
  left_join(prod_eia_crudo %>% 
              select(anio, crudo_eia = crude_oil_condensate)) %>% 
   mutate(prod_crudo =  case_when(anio < 1993 ~ crudo_anuario,
                                  anio > 1992 ~ crudo_sesco),
          unidad = "barriles", 
...
#Existencias
existencias_petroleo <- read_excel("datasets/existencias_crudo.xlsx") %>%
...
ggplotly(graf_precio_mdo_mundial_crudo, width = 800, height = 600)
...
graf_precio_mdo_mundial_gas_mmbtu <- precio_mdomundial_gas_MMBTU %>% 
  ...
  labs(title = "Precios de exportación y referencia del mercado mundial del gas", 
       y = "usd/MMBTU")
ggplotly(graf_precio_mdo_mundial_gas_mmbtu, width = 800, height = 600)
...
comex_post2010 <- read_delim("datasets/comex_post2010.csv", 
    ";", escape_double = FALSE, trim_ws = TRUE, 
    skip = 5) %>% 
...
impo_expo <- read_csv("datasets/importaciones-exportaciones (1).csv")


###exportaciones pre 2010: "Exportaciones.xlsx"
expo_pre2010 <- read_delim("datasets/expo_pre2010.csv", 
    ";", escape_double = FALSE, trim_ws = TRUE) %>% 
  rename(anio = "Año",
         mes = "Mes",
         producto = Producto,
...
impo_pre2010 <- read_delim("datasets/impo_pre2010.csv", 
    ";", escape_double = FALSE, trim_ws = TRUE) %>% 
  rename(anio = "Año",
         mes = "Mes",
         producto = Producto,
...
expo_q_crudo <- comex_crudo_sesco %>%
  ...
  mutate(expo_crudo = case_when(anio < 1994 | anio == 1999 ~ expo_comtrade_crudo,
...
# writexl::write_xlsx(regalias %>% 
#   spread(.,
#          key = anio,
#          value = regalias_total), path = "resultados/regalias.xlsx")
```
...
#ejes 
subsidios_ejes <- read_excel("subsidios.xlsx") %>% 
...
# Valor total de la producción y ganancia
...
Hay dos estimaciones del Valor Bruto y Agregado de Producción, con su correspondiente Consumo Interemedio, los cuales corresponden a fuentes distintas. Asimismo, se deduce la plusalía total de la rama luego de descontar del Valor Agregado el respectivo consumo intermedio, masa salarial, impuestos específicos y depreciación de capital.
...
#### Fuentes elegidas para la construcción de series 
...
### Estimación por Cuentas Nacionales
```{r echo=FALSE, message=FALSE, warning=FALSE}
# coeficientes de requerimientos directos

# extraido de MIP 97 a partir de CI/VBP 
# coef_req_directos <-  0.272264638

# extraido de cuadro de utilizacion 2004 a partir de CI/VBP
coef_tecnico <-  0.369104302

coef_tecnico_97 <- 0.374125887
...
COU <- read_excel("C:/Archivos/Datos/MIP/sh_cou_06_16.xls", 
  ...
  rename(sector = "...1",
         cpc = "...2") %>% 
  filter(str_detect(sector, "\\d") & !(str_detect(sector, "^N")))

servicio_s_extraccion <-  sum(COU[12])/sum(COU[11])
nrow(COU)
```


```{r echo=FALSE, message=FALSE, warning=FALSE}
vbp_va_ci_ccnn <- read_excel("C:/Archivos/Datos/Hidrocarburos/Mecon/base-mineria-e-hidrocarburos cuentas nacionales.xls",
                        sheet = "Cuentas Nacionales", skip = 5) %>% 
  rename(anio = "Año",
         periodo ="Período",
  ...
  ungroup() %>% 
  mutate(unidad = "Millones de pesos corrientes",
         coef_req_extr_y_serv =ci_extraccion_y_servicios_hidrocarburos/vbp_extraccion_y_servicios_hidrocarburos,
         coef_req_extr = ci_extraccion_hidrocarburos/vbp_extraccion_hidrocarburos) %>% 
  filter(!(is.na(coef_req_extr_y_serv)))
vbp_va_ci_ccnn

mean_coef_req_extr_ccnn <- mean(vbp_va_ci_ccnn$coef_req_extr)
mean_coef_req_extr_ccnn_b <- mean(sum(vbp_va_ci_ccnn$ci_extraccion_hidrocarburos)/
                                  sum(vbp_va_ci_ccnn$vbp_extraccion_hidrocarburos))

```

### Estimaciones en base a fuentes recopiladas. Metodología propia y CCNN
...
* $VBP_{propia}$ = Valor Bruto de la Producción, estimación propia
* $VBP_{CCNN}$ = Valor Bruto de la Producción,  criterio de las CCNN
* $Pext_{petróleo}$ = Precio de exportación/referencia internacional del petróleo crudo
* $Pext_{gas}$ = Precio de exportación/referencia internacional del gas natural
...
* $CI_{propia}$ = Consumo Intermedio, estimación propia
* $VA_{propia}$ = Valor Agregado, estimación propia 
...
* $MS_{propia}$ = Masa Salarial, estimación propia
...
* $Dep$ = Depreciaciones (cuenta gastos)
...
*  $PV_{propia}$ = Plusvalía (*Excedente Bruto de Explotación*)
...
El consumo de capital fijo surge de aplicar la tasa de depreciación promedio resultante de los balances de YPF (1998 - 2018) al stock de capital adelantado estimado (ver apartado *Estimación de stock de capital a partir de la evolución de pozos perforados*) . 



Corrección VPB

Cuadro utilizacion de oferta
peso de los servicios = Servicios petroleo/extraccion = 10,7% (2)

años 2004-2011
aplicamos coef a base mineria/hidrocarburos CCNN
VBP extraccion( CCNN) = VBP extraccion y servicios (CCNN) * 0,893 (1)

1910 - 2018
armar indice VBP criterio CCNN para mover (1) los años que nos faltan (1910 - 2004 y 2012 -2018) 
deberia ser = que aplicar coef (2) a VBP
...
precios_y_cantidades <- prod_merge_crudo %>% 
  ...
  left_join(precio_mdomundial_gas_MMBTU %>% 
              select(anio, unidad_precio_gas = unidad, precio_externo_gas), by = "anio") %>% 
  left_join(precio_interno_gas_mmbtu_usd %>% 
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
vbp_va_ci_propia <- precios_y_cantidades %>% 
  mutate(crudo_mdo_interno = prod_crudo - expo_crudo,
         expo_gas = replace_na(expo_gas, 0 ),
         gas_mdo_interno = prod_gas - expo_gas,
         unidad_valor_produccion = "Millones de pesos corrientes",
         vbp_criterio_ccnn = ((crudo_mdo_interno*precio_crudo_mdoint+expo_crudo*precio_me_crudo
                               + gas_mdo_interno * precio_gas_mdoint +expo_gas*precio_externo_gas )*tcc)/10^6,
         valor_bruto_crudo = (prod_crudo*precio_me_crudo*tcp)/10^6,
         valor_bruto_gas =   (prod_gas * precio_externo_gas*tcp)/10^6,
         vbp_criterio_propio = valor_bruto_crudo + valor_bruto_gas, 
         consumo_intermedio = vbp_criterio_ccnn * coef_tecnico,
         valor_agregado = vbp_criterio_propio - consumo_intermedio )
vbp_va_ci_propia

indice_vbp_criterio_ccnn <- vbp_va_ci_propia %>% 
  ungroup() %>% 
  mutate(vbp_criterio_ccnn_04 = generar_indice(vbp_criterio_ccnn,
                                               fecha = anio,
                                               fecha_base = 2004),
         vbp_criterio_ccnn_12 = generar_indice(vbp_criterio_ccnn,
                                               fecha = anio,
                                               fecha_base = 2012)) %>% 
  select(anio, vbp_criterio_ccnn_04,vbp_criterio_ccnn_12)
...
* MTEySS (Base Minería e Hidrocarburos de las Cuentas Nacionales)
* [Ministerio de Trabajo, Empleo y Seguridad Social - Observatorio de Empleo y Dinámica Empresarial (OEDE)](http://www.trabajo.gob.ar/estadisticas/oede/estadisticasnacionales.asp)
* Falta EPH
...
masa_salarial_hidrocarburos <- masa_salarial_ccnn %>% 
  select(anio, unidad = unidad_masa_salarial, masa_salarial_ccnn = masa_salarial_total) %>% 
  full_join(masa_salarial_oede %>% 
              select(anio, unidad = unidad_masa_salarial, 
                     masa_salarial_oede =masa_salarial_total)) %>% 
...
## Plusvalía total del sector

```{r echo=FALSE, message=FALSE, warning=FALSE}
#con base de cuentas nacionales
pv_hidrocarburos_ccnn <- vbp_va_ci_ccnn %>% 
  left_join(masa_salarial_hidrocarburos %>% 
              select(anio,  unidad,
                     masa_salarial = masa_salarial_oede), by = c("anio", "unidad")) %>% 
  mutate(excedente_bruto_explotacion = va_extraccion_y_servicios_hidrocarburos -masa_salarial) %>% 
  rename(valor_bruto_produccion = vbp_extraccion_hidrocarburos,
         consumo_intermedio = ci_extraccion_hidrocarburos,
         valor_agregado= va_extraccion_hidrocarburos)



#con base de VBP de elaboracion propia
mean_coef_ms <-  mean((pv_hidrocarburos_ccnn %>% 
  group_by(anio) %>% 
  summarise(coef_masa_salarial = masa_salarial/valor_bruto_produccion) %>% 
  filter(!(is.na(coef_masa_salarial))))$coef_masa_salarial)

median_coef_ms <-  median((pv_hidrocarburos_ccnn %>% 
  group_by(anio) %>% 
  summarise(coef_masa_salarial = masa_salarial/valor_bruto_produccion) %>% 
  filter(!(is.na(coef_masa_salarial))))$coef_masa_salarial)
...
#restimacion de VBP criterio CCNN
vbp_ccnn <- vbp_va_ci_ccnn %>% 
  select(anio,unidad, vbp_extraccion_hidrocarburos, va_extraccion_hidrocarburos, ci_extraccion_hidrocarburos) %>% 
  full_join(indice_vbp_criterio_ccnn, by = "anio") %>% 
  arrange(anio) %>% 
  mutate(vbp_extraccion_ccnn = case_when(
    anio < 2004 ~ vbp_va_ci_ccnn$vbp_extraccion_hidrocarburos[vbp_va_ci_ccnn$anio == 2004] *
      vbp_criterio_ccnn_04,
    anio > 2012 ~ vbp_va_ci_ccnn$vbp_extraccion_hidrocarburos[vbp_va_ci_ccnn$anio == 2012]  *
      vbp_criterio_ccnn_12,
    T ~ vbp_extraccion_hidrocarburos),
    ci_extraccion_ccnn = vbp_extraccion_ccnn*coef_tecnico,
    # ci_extraccion_ccnn = vbp_extraccion_ccnn*mean_coef_req_extr_ccnn,
    va_extraccion_ccnn = vbp_extraccion_ccnn- ci_extraccion_ccnn,
    unidad = "Millones de pesos corrientes") %>%
  left_join(masa_salarial_hidrocarburos %>% 
              select(anio,  unidad, ms_ccnn = masa_salarial_oede),
            by = c("anio", "unidad")) %>%
  left_join(stock_estimado %>%
               left_join(ipc_promedio %>% select(anio, ipc_18)) %>%
              mutate(consumo_k_fijo = valor * ipc_18 * consumo_k_fijo_ypf) %>% 
               select(anio, consumo_k_fijo)) %>% 
  mutate(ms_ccnn = case_when(is.na(ms_ccnn) ~vbp_extraccion_ccnn*mean_coef_ms,
                                   T ~ ms_ccnn),
         ebe_ccnn = va_extraccion_ccnn -ms_ccnn -consumo_k_fijo,
         fuente = "CCNN reestimado") %>% 
   select(anio, fuente, unidad, 
          vbp_extraccion_ccnn, ci_extraccion_ccnn, va_extraccion_ccnn, 
          ms_ccnn, consumo_k_fijo, ebe_ccnn) %>% 
  arrange(anio)
vbp_ccnn
```
```{r}
read_csv("C:/Archivos/Datos/Balances/gas y petroleo_datos/datos_r/resultados/balances_arg.csv", 
    ...
    filter(empresa == "YPF") %>% 
    group_by(empresa) %>% 
    mutate(tasa_depreciacion = depreciaciones/ppye) %>% select(fecha, empresa, tasa_depreciacion)
```
...
```{r echo=FALSE, message=FALSE, warning=FALSE}
# propia
pv_hidrocarburos_propia <-  vbp_va_ci_propia %>% 
  select(anio, unidad = unidad_valor_produccion, 
         vbp_criterio_propio, consumo_intermedio,valor_agregado ) %>% 
   left_join(stock_estimado %>%
               left_join(ipc_promedio %>% select(anio, ipc_18)) %>%
              ...
              mutate(consumo_k_fijo = valor * ipc_18 * consumo_k_fijo_ypf) %>% 
               select(anio, consumo_k_fijo)) %>% 
  mutate(
    # masa_salarial = valor_agregado * coef_salario,
    masa_salarial = vbp_criterio_propio * mean_coef_ms,
        excedente_bruto_explotacion =  valor_agregado - masa_salarial - consumo_k_fijo)
  
# PV CCNN y propia
pv_hidrocarburos <- pv_hidrocarburos_ccnn %>% 
  rename(vbp_ccnn = valor_bruto_produccion,
         va_ccnn = valor_agregado,
         ci_ccnn = consumo_intermedio,
         ms_ccnn = masa_salarial,
         coef_req_ccnn = coef_req_extr,
         ebe_ccnn= excedente_bruto_explotacion) %>% 
  full_join(pv_hidrocarburos_propia %>% 
              select(anio, unidad ,
                vbp_criterio_propio, ci_criterio_propio = consumo_intermedio,
                va_criterio_propio = valor_agregado , consumo_k_fijo_propio = consumo_k_fijo))  %>%
  # left_join(vbp_va_ci_propia %>% 
  #             select(anio, vbp_criterio_ccnn )) %>% 
  left_join(vbp_ccnn %>% 
              select(anio, vbp_criterio_ccnn = vbp_extraccion_ccnn,
                     ci = ci_extraccion_ccnn, ms = ms_ccnn)) %>% 
  arrange(anio) %>% 
  mutate(vbp = vbp_criterio_propio ,
         # ci = case_when(is.na(ci_ccnn) ~ vbp_criterio_ccnn * coef_tecnico  , T ~  ci_ccnn),
         va = vbp_criterio_propio - ci,
         # ms = case_when(is.na(ms_ccnn) ~ vbp_criterio_ccnn* mean_coef_ms, T ~  ms_ccnn),
         ebe =  va - ms -consumo_k_fijo_propio)
          

###
valor_total_produccion <- pv_hidrocarburos_ccnn %>%  # no tiene descontado servicios
 mutate(fuente = "CCNN") %>% 
  select(anio, unidad, fuente,valor_bruto_produccion,valor_agregado,
         consumo_intermedio, masa_salarial, excedente_bruto_explotacion) %>% 
  gather(key = variable,
         value = valor,
         4:ncol(.)) %>% 
  bind_rows(pv_hidrocarburos_propia %>% 
              mutate(fuente = "Criterio Propio") %>% 
              select(anio, unidad, 
                     fuente,
                     valor_bruto_produccion = vbp_criterio_propio,
                     everything(.)) %>% 
              gather(key = variable,
                      value = valor,
                      4:ncol(.))) %>%
  bind_rows(pv_hidrocarburos %>% #tiene descontado  servicios
              mutate(fuente = "Criterio propio con VBP CCNN") %>% 
              select(anio, unidad, fuente, valor_bruto_produccion = vbp, 
                   consumo_intermedio = ci, 
                   valor_agregado= va, 
                   masa_salarial = ms, 
                   excedente_bruto_explotacion = ebe,
                   consumo_k_fijo= consumo_k_fijo_propio) %>% 
               gather(key = variable,
                      value = valor,
                      4:ncol(.))) %>% 
  bind_rows(vbp_va_ci_propia %>% # no tiene descontado servicios
              left_join(stock_estimado %>%  
                          select(-unidad),
                          # filter(variable == "ppye_estimado_bolsar"),
                        by = "anio") %>%
              ...
              mutate(consumo_k_fijo = valor * ipc_18 * consumo_k_fijo_ypf,
                     ...
                     valor_agregado = vbp_criterio_ccnn - consumo_intermedio,
                     masa_salarial = mean_coef_ms * vbp_criterio_ccnn,
                     excedente_bruto_explotacion =valor_agregado-  masa_salarial-consumo_k_fijo,
                     unidad = "Millones de pesos corrientes") %>%
              select(anio, unidad, fuente,
                    valor_bruto_produccion = vbp_criterio_ccnn,
                   consumo_intermedio,
                   valor_agregado,
                   masa_salarial,
                   excedente_bruto_explotacion , consumo_k_fijo) %>%
              gather(key = variable,
                      value = valor,
                      4:ncol(.))) %>%
  bind_rows(vbp_ccnn %>% # CCNN Reestimado, tiene descontado serivicios
              #select(-consumo_k_fijo) %>% 
              rename(valor_bruto_produccion = vbp_extraccion_ccnn, 
                   consumo_intermedio = ci_extraccion_ccnn,
                   valor_agregado =va_extraccion_ccnn,
                   masa_salarial =ms_ccnn,
                   excedente_bruto_explotacion =ebe_ccnn) %>% 
              gather(key = variable,
                      value = valor,
                      4:ncol(.))) %>%
  left_join(ipc_promedio %>% select(anio, ipc_18)) %>% 
...
graf_valor_produccion <- valor_total_produccion %>% 
  ...
  labs(title = "Valor de la Producción hidrocarburífera", 
       ...
       y = "Millones de pesos de 2018")+
  facet_wrap(~variable)
ggplotly(graf_valor_produccion, width = 750, height = 500)
```


```{r echo=FALSE}
vbp_va_ci_propia %>% 
  select(anio, valor_bruto_crudo, valor_bruto_gas) %>% 
  ...
  geom_col()+
   labs(title = "Valor Bruto de la Producción hidrocarburífera", 
       subtitle = "Desagregación en petróleo y gas", 
       y = "Millones de pesos de 2018")
```
...
ggplotly(graf_tg_emp, width = 800, height = 600)
```
...
#Estimación de tasa de ganancia total rama

tasa_ganancia_rama_flujo <- valor_total_produccion %>%
...
graf_tg_rama_1 <- tasa_ganancia_rama_flujo %>% 
  filter(anio >1960) %>% 
  ggplot(aes(anio, tasa_ganancia, color =fuente_pozo ))+
  geom_line(alpha = 0.5)+
  geom_point()+
  scale_y_continuous(labels = scales::percent)+
  theme(axis.text.x = element_text(angle = 90),
        legend.position = "bottom")+
  labs(title = "Tasa de ganancia hidrocarburífera con PPyE a partir de flujo de pozos(1960 - 2018)")+
...
renta_tg <-  tasa_ganancia_rama_flujo %>%
  select(anio,  stock_seleccionado, fuente_pozo, ppye , tasa_ganancia) %>% 
  ...
  mutate(renta_con_tg_jic = ppye *(tg_hidrocarburos - tg_indu_jic),
         renta_con_tg_em = ppye *(tg_hidrocarburos - tg_indu_em),
         union_tg = case_when(anio < 1993 ~ tg_indu_jic,
...
renta_produccion_balances <-  renta_empresas_balances %>% 
  filter(!(empresa %in% c("chevron", "petrobras_glob" )),
...
ggplotly(graf_renta_emp, width = 800, height = 600)
...
ejercicio <- valor_total_produccion %>% 
  filter(variable == "valor_agregado") %>% 
...
ggplotly(graf_relacion, width = 800, height = 600)
...
renta_dif_precios_crudo <- prod_merge_crudo %>% 
  ...
  mutate(renta_dif_precios_crudo = prod_mdo_interno*precio_me_crudo*tcp - prod_mdo_interno*precio_crudo_mdoint*tcc,
         ...
         unidad_precio = "usd", unidad_renta = "pesos_corr") %>% 
...
retenciones_campodonico <- read_excel("datasets/retenciones.xlsx", 
    sheet = "usd") %>% 
  left_join(tcp_anual %>% select(-TCP)) %>% 
...
retenciones_crudo <- read_excel("datasets/retenciones.xlsx") %>%
...
## Renta total a partir de descuentos sobre el VBP

```{r echo=FALSE}
# knitr::opts_chunk$set(echo= TRUE,fig.width = 120,fig.height = 400)


renta_hidrocarburifera_total <- renta_tg %>% 
  select(anio, stock_seleccionado, ppye,
         tg_hidrocarburos, union_tg) %>% 
  left_join(valor_total_produccion %>% 
              filter(variable == "excedente_bruto_explotacion") %>% 
              rename(excedente_bruto_explotacion = valor) %>% 
              select(-c(variable, unidad)) %>% 
              rename(fuente_gcia = fuente)) %>% 
  ...
  mutate(renta_con_union_tg = ppye* (tg_hidrocarburos - union_tg),
         gcia_normal_hidrocarburos = ppye * union_tg,
         renta_total = excedente_bruto_explotacion - gcia_normal_hidrocarburos -subsidios )
renta_hidrocarburifera_total
...
renta_hidrocarburifera_total %>%
  ...
  labs(title = "Renta hidrocarburífera total a partir de estimación propia del VBP",
       y = "Millones de pesos de 2018")+
  theme(axis.text.x = element_text(angle = 90))+
  facet_wrap(~fuente_gcia)



renta_total_pbi_pv <- renta_hidrocarburifera_total %>% 
...
graf_renta_total_pbi_pv <- renta_total_pbi_pv %>% 
  select(anio, fuente_gcia, renta_pv,renta_pbi ) %>%
  ...
  labs(title = "Renta hidrocarburífera total sobre PBI y Ganancias Totales",
       sbutitle = "A partir de estimación propia del VBP",
       y = "%")+
  scale_y_continuous(labels=scales::percent_format(accuracy = 2))+
  facet_grid(fuente_gcia~tipo_renta )

# graf_renta_total_pbi_pv
ggplotly(graf_renta_total_pbi_pv, height = 400, width = 600)

```


## Renta total a partir de suma de mecanismos

```{r echo=FALSE}
renta_pbi_pv <- renta_dif_precios_crudo %>% 
  ...
  ungroup() %>% 
  # left_join(renta_produccion_balances %>% select(anio, renta_empresas_desp_impuestos)) %>% 
  # left_join(renta_hidrocarburifera_total %>%
  #             filter(stock_seleccionado == "ppye_bolsar") %>%
  #             select(anio, renta_empresas = renta_con_union_tg) %>% 
  ...
  mutate(renta_dif_precios_gas = renta_dif_precios_gas /ipc_18,
         ...
         retenciones_crudo_jk = retenciones_crudo_jk / ipc_18, 
         subsidios = subsidios/ipc_18,
         # renta_empresas_desp_impuestos = renta_empresas_desp_impuestos/ipc_18,
         renta_total = renta_dif_precios_gas+ renta_dif_precios_crudo +
           renta_sobrevaluacion_crudo + renta_sobrevaluacion_gas +
           regalias_total +  retenciones_crudo_jk - subsidios,
  ...
  select(anio, unidad, ipc_18, everything(.)) %>% 
...
#Renta total
graf_renta_total <- renta_pbi_pv %>%
  filter(anio >1960) %>% 
  select(-c(unidad, ipc_18, renta_total, tcc, tcp, sobrevaluacion)) %>% 
  gather(key = tipo_renta,
         value = valor,
         2:8) %>% 
  ggplot(aes(anio, valor, color = tipo_renta, fill = tipo_renta))+
...
# OLVIDAR
# graf_renta_total_comparacion_b <- renta_usd %>% 
...
#   theme(legend.position = "bottom")+
#   labs(title = "Renta hidrocarburífera total (petróleo y gas)", 
#        subtitle = "Con renta total propia metodología propia", 
#        y = "Millones de USD")+
#   theme_classic()+
#   scale_y_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE))
# graf_renta_total_comparacion_b


#Renta TCC
graf_renta_total_usd <- renta_pbi_pv %>% 
...
#comparacion con FR
graf_comparacion_rentas <- renta_pbi_pv %>% 
  mutate(fuente = "Propia via suma de mecanismos") %>% 
  select(anio, unidad,fuente,  renta_total ) %>% 
  bind_rows(renta_total_pbi_pv %>% 
                filter( fuente_gcia == "Propia") %>%
...
#sobre pbi y pv
graf_renta_total_pv <- renta_pbi_pv %>% 
...
costos <- produccion_total %>% 
  select(anio, unidad_produccion=unidad, produccion_total=produccion_total_bep) %>% 
  left_join(valor_total_produccion %>% 
              filter(variable %in% c("consumo_intermedio", "masa_salarial","consumo_k_fijo" )) %>%
              spread(key=variable, value=valor)) %>% 
  left_join(renta_hidrocarburifera_total %>% select(anio, stock_seleccionado, 
                                                    ppye, tg_normal=union_tg, renta_total)) %>% 
  mutate(costos_totales = consumo_intermedio +masa_salarial + consumo_k_fijo,
...
renta_usd <- renta_pbi_pv %>% 
  select(anio,tcc, ipc_18, retenciones_crudo_jk, regalias_total, contains("renta")) %>% 
  select( -(12:ncol(.))) %>% 
  filter(anio > 1962) %>%
  mutate( retenciones_crudo_jk= retenciones_crudo_jk*ipc_18/tcc,       
          ...
          # renta_empresas_desp_impuestos = renta_empresas_desp_impuestos*ipc_18/tcc,
          renta_total = (renta_total*ipc_18) / tcc,
          unidad = "Millones de USD",
         autor = "Propia") %>% 
  rename(retenciones = retenciones_crudo_jk,
         regalias = regalias_total
          # renta_empresas = renta_empresas_desp_impuestos,
  ...
  select(anio, unidad, autor, everything(.), 
         -c(ipc_18, tcc, renta_expo_sobrevaluada_crudo, renta_expo_sobrevaluada_gas,
            renta_diferencial_precios_gas, renta_diferencial_precios_crudo)) %>% 
...
graf_renta_total_comparacion <- renta_usd %>% 
  # filter(!(autor == "Propia" & tipo_de_renta == "renta_total")) %>%
  # mutate(tipo_de_renta= case_when(tipo_de_renta =="renta_diferencial_precios" &
  #                                   autor == "Propia" ~ "renta_total",
  #                                 T ~ tipo_de_renta)) %>%
...
writexl::write_xlsx(list(produccion_crudo = prod_merge_crudo, 
                         precio_interno_crudo = precio_mi_crudo, 
                         exportacion_crudo = expo_q_crudo, 
                         precios_externo_crudo = precios_referencia_y_expo_crudo,
                         produccion_gas = prod_merge_gas_MMBTU, 
                         precio_interno_gas =precio_interno_gas_mmbtu_usd, 
                         exportacion_gas = expo_q_gas, 
                         precio_externo_gas = precio_mdomundial_gas_MMBTU,
                         tipo_cambio = tcp_anual_b,
                         ipc = ipc_promedio),
                    path = "resultados/variables.xlsx")


writexl::write_xlsx(list(renta_total_pv_pbi = renta_pbi_pv,
                         valor_produccion = valor_total_produccion ,
                         VA_propio = vbp_va_ci_propia,
                         crudo_dif_precios_ms = renta_dif_precios_crudo, 
                         gas_dif_precios_ms = renta_diferencial_precios_gas,
                         crudo_sobrevaluacion_ms = renta_tcp_crudo,
                         gas_sobrevaluacion_ms = renta_tcp_gas,
                         renta_balances = renta_produccion_balances,
                         # crudo_precio_expo_jk = renta_precio_expo_crudo_jk,
                         # gas_dif_precios_expo_jk = renta_precio_expo_gas_jk,
                         estimacion_jk = renta_hidrocarburos_jk), 
                    path = "resultados/renta_hidrocarburos_total.xlsx")


writexl::write_xlsx(renta_total_pbi_pv,
                    path = "resultados/renta_total_PxQ.xlsx")
...
writexl::write_xlsx(list(afip = stock_afip,
                    ...
                    path = "resultados/stock_petroleo.xlsx")

writexl::write_xlsx(list("TG indice flujo pozos"= tasa_ganancia_rama_flujo, 
                         # "TG indice flujo stock" = tasa_ganancia_rama_stock,
                         "renta empresas" = renta_tg), 
                    path = "resultados/tasa de ganancia y renta hidrocarburífera.xlsx")


print("Mateo, el código terminó de correr")
...
Recortes viejos que no se pq se realizaron

<!-- ### Renta por exportaciones a tipo de cambio sobrevaluado -->
<!-- #### Crudo -->
<!-- ```{r message=FALSE, warning=FALSE} -->
<!-- renta_tcp_crudo <- comex_crudo_sesco %>% -->
<!--   filter(variable == "exportacion", -->
<!--          unidad == "usd") %>% -->
<!--   rename(expo_crudo_usd = valor) %>% -->
<!--   ungroup() %>% -->
<!--   select(-c(variable, unidad, producto)) %>% -->
<!--   left_join(tcp_anual, by = "anio") %>% -->
<!--   mutate(renta_sobrevaluacion_crudo_pesos = expo_crudo_usd*TCP - expo_crudo_usd*TCC ) -->
<!-- renta_tcp_crudo -->
<!-- ``` -->

<!-- #### Gas -->
<!-- ```{r message=FALSE, warning=FALSE} -->
<!-- renta_tcp_gas <- comex_gas %>% -->
<!--   filter(variable == "exportacion", -->
<!--          unidad == "usd") %>% -->
<!--   rename(expo_gas_usd = valor) %>% -->
<!--   ungroup() %>% -->
<!--   select(-c(variable, unidad, producto)) %>% -->
<!--   left_join(tcp_anual, by = "anio") %>% -->
<!--   mutate(renta_sobrevaluacion_gas_pesos = expo_gas_usd*TCP - expo_gas_usd*TCC ) -->
<!-- renta_tcp_gas -->
<!-- ``` -->
...
<!-- ### Renta por regalías -->
<!-- ```{r message=FALSE, warning=FALSE} -->
<!-- renta_regalias <- regalias %>% -->
<!--   left_join(tcp_anual, by = "anio") %>% -->
<!--   mutate(regalias_pesos = case_when(moneda == "usd" ~ regalias_total * TCC, -->
<!--                                     T ~ regalias_total), -->
<!--          moneda = case_when(moneda == "usd" ~ "ars", -->
<!--                             T ~ moneda)) %>% -->
<!--   group_by(anio) %>% -->
<!--   summarise(regalias_pesos = sum(regalias_pesos)) -->
<!-- renta_regalias -->
<!-- ``` -->

<!-- ## Renta total -->
<!-- ```{r message=FALSE, warning=FALSE} -->
<!-- renta_jk <- read_delim("renta_jk.csv", ";", -->
<!--     escape_double = FALSE, trim_ws = TRUE) %>% -->
<!--   rename(anio = X1) -->

<!-- # renta_hidrocarburos <- renta_regalias %>% -->
<!-- #   left_join(renta_tcp_gas %>% -->
<!-- #               select(anio, renta_sobrevaluacion_gas_pesos)) %>% -->
<!-- #   left_join(renta_tcp_crudo %>% -->
<!-- #               select(anio, renta_sobrevaluacion_crudo_pesos))  %>% -->
<!-- #   left_join(renta_dif_precios_crudo %>% -->
<!-- #               select(anio, renta_dif_precios_crudo_pesos)) %>% -->
<!-- #   left_join(renta_dif_precios_gas %>% -->
<!-- #               select(anio, renta_dif_precios_gas_pesos))  %>% -->
<!-- #   mutate(renta_total_pesos = sum(regalias_pesos, -->
<!-- #                                  renta_sobrevaluacion_gas_pesos, renta_sobrevaluacion_crudo_pesos, -->
<!-- #                                  renta_dif_precios_crudo_pesos, renta_dif_precios_gas_pesos, -->
<!-- #                                  na.rm = T)) %>% -->
<!-- #   left_join(renta_jk) %>% -->
<!-- #   mutate(diferencia = (renta_total_pesos/renta_jk) - 1) -->
<!-- # -->
...
<!-- # write.csv(renta_hidrocarburos, "renta_hidrocarburos.csv") -->
...
<!-- ```{r message=FALSE, warning=FALSE} -->
<!-- graf_renta <- renta_hidrocarburos %>% -->
<!--   gather(., -->
<!--          key = variable, -->
<!--          value = valor, -->
<!--          2:ncol(.)) %>% -->
<!--   filter(!(variable %in% c("renta_total_pesos", "renta_jk", "diferencia"))) %>% -->
<!--   ggplot(aes(anio, valor, fill = variable))+ -->
<!--   geom_col(position = "stack") -->
<!-- graf_renta -->
<!-- ``` -->

Recorte de estimacion de kta a partir de pozos
<!-- #### KTA. Con stock de pozos en extracción efectiva -->
<!-- ```{r eval=FALSE, message=FALSE, warning=FALSE, include=FALSE} -->
<!-- #KTA -->
<!-- kta_anio_base_bolsar <- (stock_rama %>%  -->
<!--   filter(variable == "KTA" & sector == "produccion") %>% -->
<!--   select(anio, unidad, KTA = valor))$KTA[10] -->

<!-- kta_anio_base_afip <- (stock_rama %>%  -->
<!--   filter(variable == "KTA" & sector == "extraccion_petroleo_gas") %>% -->
<!--   select(anio, unidad, KTA = valor))$KTA[6] -->

<!-- pozos_stock_kta <- stock_rama %>%  -->
<!--   filter(variable == "KTA" & sector %in% c("produccion", "extraccion_petroleo_gas")) %>% -->
<!--   select(anio, unidad, KTA = valor, fuente) %>%  -->
<!--   full_join(pozos_iapg_centenario %>%  -->
<!--               mutate( indice_pozos_06 = generar_indice(serie= totales , -->
<!--                                         fecha = anio, -->
<!--                                         fecha_base = 2006)) %>%  -->
<!--               select(anio, indice_pozos_06))  -->

<!-- ejercicio_fd_stock <- pozos_stock_kta %>% -->
<!--   group_by(anio, unidad, fuente) %>% -->
<!--   summarise(kta_estimado_bolsar = kta_anio_base_bolsar * indice_pozos_06, -->
<!--             kta_estimado_afip = kta_anio_base_afip * indice_pozos_06)  %>%  -->
<!--   ungroup() %>%  -->
<!--   select(-c(unidad,  fuente)) %>%  -->
<!--   gather(key =variable, -->
<!--                       value = valor,  -->
<!--                       2:3) %>%  -->
<!--   bind_rows( stock_rama %>%  -->
<!--                filter(variable == "KTA" & -->
<!--                         sector %in% c("produccion", "extraccion_petroleo_gas")) %>% -->
<!--                select(anio,  variable = fuente, valor )) %>%  -->
<!--   mutate(unidad = "Millones de pesos 2018") -->
<!-- # ejercicio_fd_stock -->


<!-- graf_ejercicio_fg <- ejercicio_fd_stock %>%  -->
<!--   ggplot(aes(anio, valor, color= variable))+ -->
<!--   geom_line()+ -->
<!--   scale_y_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE))+ -->
<!--   labs(title = "KTA a partir de balances, AFIP y estimado por indice de stock pozos (base 2006)", -->
<!--        y = "Millones de pesos de 2018")+ -->
<!--   theme(legend.position = "bottom") -->


<!-- cor(x= pozos_stock_kta$KTA[pozos_stock_kta$fuente == "AFIP"] ,  -->
<!--     y= pozos_stock_kta$indice_pozos_06[pozos_stock_kta$fuente == "AFIP"], -->
<!--      method = "pearson", use = "complete.obs") -->
<!-- # 0.5488576 -->

<!-- cor(x= pozos_stock_kta$KTA[pozos_stock_kta$fuente == "Bolsar"] ,  -->
<!--     y= pozos_stock_kta$indice_pozos_06[pozos_stock_kta$fuente == "Bolsar"], -->
<!--      method = "pearson", use = "complete.obs") -->
<!-- # 0.8875074 -->

<!-- #sin filtrar la variable de fuente -->
<!-- # 0.4179967 con KTA -->
<!-- # 0.4241663 con ppye -->
...
<!-- #### KTA. Con pozos terminados por año -->
<!-- ```{r eval=FALSE, message=FALSE, warning=FALSE, include=FALSE} -->
<!-- #KTA -->
<!-- pozos_flujo_kta <- stock_rama %>%  -->
<!--   filter(variable == "KTA" & sector %in% c("produccion", "extraccion_petroleo_gas")) %>% -->
<!--   select(anio, unidad, KTA = valor, fuente) %>%  -->
<!--   full_join(comparacion_pozos %>%  -->
<!--               mutate( indice_pozos_06 = generar_indice(serie= pozos_cargados_sec_energia , -->
<!--                                         fecha = anio, -->
<!--                                         fecha_base = 2006)) %>%  -->
<!--               select(anio, indice_pozos_06))  -->

<!-- ejercicio_fd_flujo <-pozos_flujo_kta %>% -->
<!--   group_by(anio, unidad, fuente) %>% -->
<!--   summarise(kta_estimado_bolsar = kta_anio_base_bolsar * indice_pozos_06, -->
<!--             kta_estimado_afip = kta_anio_base_afip * indice_pozos_06)  %>%  -->
<!--   ungroup() %>%  -->
...
<!--                         sector %in% c("produccion", "extraccion_petroleo_gas")) %>% -->
<!--               select(anio,  variable = fuente, valor ) %>%  -->
<!--               mutate(variable = case_when(variable == "AFIP" ~ "KTA_AFIP", -->
<!--                                           variable =="Bolsar" ~ "KTA_Bolsar", -->
<!--                                           T ~ variable)))%>%  -->
<!--   mutate(unidad = "Millones de pesos 2018") -->


<!-- writexl::write_xlsx(list("con pozos perforados por año"= ejercicio_fd_flujo,  -->
<!--                          "con stock de pozos" = ejercicio_fd_stock), path = "resultados/estimacion de stock a partir de pozos.xlsx") -->

<!-- #Sin filtar la variable fuente -->
<!-- cor(x= pozos_flujo_kta$KTA, y=pozos_flujo_kta$indice_pozos_06, -->
<!--      method = "pearson", use = "complete.obs") -->
<!-- # 0.3066807 con KTA -->
<!-- # 0.3523185 con ppye -->

<!-- cor(x= pozos_flujo_kta$KTA[pozos_flujo_kta$fuente == "AFIP"] ,  -->
<!--     y= pozos_flujo_kta$indice_pozos_06[pozos_flujo_kta$fuente == "AFIP"], -->
<!--      method = "pearson", use = "complete.obs") -->
<!-- # -0.1516902 -->

<!-- cor(x= pozos_flujo_kta$KTA[pozos_flujo_kta$fuente == "Bolsar"] ,  -->
<!--     y= pozos_flujo_kta$indice_pozos_06[pozos_flujo_kta$fuente == "Bolsar"], -->
<!--      method = "pearson", use = "complete.obs") -->
<!-- # 0.5697615 -->


<!-- graf_ejercicio_fg <- ejercicio_fd_flujo %>%  -->
<!--   ggplot(aes(anio, valor, color= variable))+ -->
<!--   geom_line()+ -->
<!--   scale_y_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE))+ -->
<!--   labs(title = "KTA a partir de balances, AFIP y estimado por indice de flujo pozos (base 2006)", -->
<!--        y = "Millones de pesos de 2018")+ -->
<!--   theme(legend.position = "bottom") -->
<!-- # ggplotly(graf_ejercicio_fg) -->
<!-- graf_ejercicio_fg -->
<!-- ``` -->
...
Buscar: 
- Montamat
- Einstoss Tinto A. Análisis de la Evolución reciente de la Renta Petrolera en Argentina CECE

### Producción
 * ver cantidades vendidas y no producidas. buscar si existe serie de existencias
 
### Exportacion
  # fijarse de lo de betania
  
### Precios
  #notas sobre la revisión de excels betania:
  * Diferencial de precios combustibles y crudo - ARG EEUU WTI: el precio interno en pesos parece resultante de la división entre valor y cantidad. Luego el precio interno en dolares esta en barriles pero no se sabe como se calculo ni fuente
  * Serie larga cursos de renta 1963-2011: el precio interno de la solapa recalculo diferencial de precios, no es ni sesco ni mecon pero da muy similar
  
  #precio gas bolivia: fuente de bolivia. ver el tema de transporte
  #precio de referencia de petroleo: escalante? otro?
  
  # Subsidios: JK usó fuente EJES