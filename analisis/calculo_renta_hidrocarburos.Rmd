...
tasa_ganancia_rama_flujo <- criterio_propio %>%
...
renta_hidrocarburifera_total <- renta_tg %>% 
  ...
  left_join(criterio_propio %>% 
              select(anio, excedente_bruto_explotacion =pv) )%>% 
  ...
  mutate(renta_con_union_tg = ppye* (tg_hidrocarburos - union_tg),
         ...
         renta_total = (excedente_bruto_explotacion/ipc_18) - gcia_normal_hidrocarburos -subsidios,
 ...
 left_join(stock_estimado %>%
  ...
  left_join(valor_total_produccion %>% 
              ...
              filter(variable %in% c("ci_extr", "ms_extr"),
                     fuente%in% c("Criterio CCNN", "CCNN oficial")) %>%
...
writexl::write_xlsx(list(renta_total_pv_pbi = renta_pbi_pv,
                         ...
                         va_criterio_propio = criterio_propio,
