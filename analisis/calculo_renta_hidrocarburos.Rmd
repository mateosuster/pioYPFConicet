...
$$ CI_{CCNN} = VBP\_extraccion_{CCNN} *  Coef\_tec $$
...
ccnn_oficial <- read_excel("C:/Archivos/Datos/Hidrocarburos/Mecon/base-mineria-e-hidrocarburos cuentas nacionales.xls",
  ...
  left_join(masa_salarial_hidrocarburos %>% 
              select(anio,  unidad,
                     ...
                     masa_salarial_extraccion =masa_salarial_extraccion_oede ), by = c("anio", "unidad")) %>% 
  mutate(ebe_total = va_extraccion_y_servicios_hidrocarburos - masa_salarial_total,
         ebe_extraccion = va_extraccion_hidrocarburos -masa_salarial_extraccion) %>% 
  rename(vbp_extraccion = vbp_extraccion_hidrocarburos,
         ci_extraccion = ci_extraccion_hidrocarburos,
         va_extraccion =va_extraccion_hidrocarburos,
         vbp_total= vbp_extraccion_y_servicios_hidrocarburos , 
         va_total= va_extraccion_y_servicios_hidrocarburos,
         ci_total = ci_extraccion_y_servicios_hidrocarburos)
ccnn_oficial

ccnn_oficial <- ccnn_oficial
#con base de VBP de elaboracion propia
mean_coef_ms <-  mean((ccnn_oficial %>% 
  group_by(anio) %>% 
  summarise(coef_masa_salarial = masa_salarial/vbp_extraccion) %>% 
  filter(!(is.na(coef_masa_salarial))))$coef_masa_salarial)

median_coef_ms <-  median((ccnn_oficial %>% 
  group_by(anio) %>% 
  summarise(coef_masa_salarial = masa_salarial/vbp_extraccion) %>% 
  filter(!(is.na(coef_masa_salarial))))$coef_masa_salarial)
...
ccnn_oficial %>% 
  summarise(coef_req_extr_y_serv =mean(ci_extraccion_y_servicios_hidrocarburos/vbp_extraccion_y_servicios_hidrocarburos, na.rm=T),
         coef_req_extr = mean(ci_extraccion_hidrocarburos/vbp_extraccion_hidrocarburos, na.rm=T))


```

### Estimación propia con criterio de propio y criterio CCNN
...
precios_y_cantidades <- prod_merge_crudo %>% 
  ...
  left_join(precio_mdomundial_gas_MMBTU %>% 
              select(anio, unidad_precio_gas = unidad, precio_externo_gas, precio_expo_gas_comtrade), by = "anio") %>% 
  left_join(precio_interno_gas_mmbtu_usd %>% 
...
vbp_va_ci_propia <- precios_y_cantidades %>% 
  mutate(crudo_mdo_interno = prod_crudo - expo_crudo,
         ...
         vbp_tot_criterio_ccnn = ((crudo_mdo_interno*precio_crudo_mdoint+
                                 expo_crudo*precio_me_crudo
                               + gas_mdo_interno * precio_gas_mdoint 
                               +expo_gas*precio_expo_gas_comtrade )*tcc)/10^6,
         vbp_extr_criterio_ccnn =vbp_tot_criterio_ccnn*(1-servicios_s_extraccion),
         ci_tot_criterio_ccnn = vbp_tot_criterio_ccnn *coef_tecnico_97 ,
         ci_ext_criterio_ccnn = vbp_extr_criterio_ccnn * coef_tecnico_97,
         valor_bruto_crudo = (prod_crudo*precio_me_crudo*tcp)/10^6,
         valor_bruto_gas =   (prod_gas * precio_externo_gas*tcp)/10^6,
         vbp_tot_criterio_propio = valor_bruto_crudo + valor_bruto_gas,
         va_tot_propio = vbp_criterio_propio - ci_ext_criterio_ccnn )
vbp_va_ci_propia
...
vbp_ccnn <- ccnn_oficial %>% 
  ...
  mutate(vbp_extraccion_ccnn = case_when(
    anio < 2004 ~ ccnn_oficial$vbp_extraccion_hidrocarburos[ccnn_oficial$anio == 2004] *
      vbp_criterio_ccnn_04,
    anio > 2012 ~ ccnn_oficial$vbp_extraccion_hidrocarburos[ccnn_oficial$anio == 2012]  *
      vbp_criterio_ccnn_12,
    T ~ vbp_extraccion_hidrocarburos),
    ci_extraccion_ccnn = vbp_extraccion_ccnn*coef_tecnico_97,
...
pv_hidrocarburos <- ccnn_oficial %>% 
  ...
  mutate(vbp = vbp_criterio_propio ,
         # ci = case_when(is.na(ci_ccnn) ~ vbp_criterio_ccnn * coef_tecnico_97  , T ~  ci_ccnn),
...
###
valor_total_produccion <- ccnn_oficial %>%  # no tiene descontado servicios
 mutate(fuente = "CCNN") %>% 
   full_join(pv_hidrocarburos_propia %>% 
              select(anio, consumo_k_fijo)) %>%  
  select(anio, unidad, fuente,vbp_extraccion,valor_agregado,
