...
$$VA_{propia}  = VBP_{propia} – CI\_extraccion_{CCNN} $$
...
* $CI\_extraccion_{CCNN}$ = Consumo intermedio del sector extracción, estimación con criterio CCNN
...
ccnn_oficial <- read_excel("C:/Archivos/Datos/Hidrocarburos/Mecon/base-mineria-e-hidrocarburos cuentas nacionales.xls",
  ...
  left_join(masa_salarial_hidrocarburos %>% 
              select(anio,  unidad,
                     ms_total = masa_salarial_total_oede,
                     ms_extraccion =masa_salarial_extraccion_oede ), by = c("anio", "unidad")) %>% 
  mutate(ebe_total = va_extraccion_y_servicios_hidrocarburos - ms_total,
         ebe_extraccion = va_extraccion_hidrocarburos -ms_extraccion) %>% 
...
mean_coef_ms <-  ccnn_oficial %>% 
  summarise(coef_ms_tot = mean(ms_total/vbp_tot,na.rm= T),
              coef_ms_extr= mean(ms_extraccion/vbp_extr ,na.rm= T))

median_coef_ms <-  ccnn_oficial %>% 
  summarise(coef_ms_tot = median(ms_total/vbp_tot,na.rm= T),
              coef_ms_extr= median(ms_extraccion/vbp_extr ,na.rm= T))
...
criterio_ccnn <- precios_y_cantidades %>% 
  ...
  mutate(crudo_mdo_interno = prod_crudo - expo_crudo,
        ...
        # criterio CCNN
         ...
         ci_tot = vbp_tot *coef_tecnico_97$`Extracción de petróleo, gas, carbón y uranio` ,
         ci_extr = vbp_extr * coef_tecnico_97$`Extracción de petróleo, gas, carbón y uranio`,
...
empalme_ccnn <- ccnn_oficial %>% 
  ...
  mutate(vbp_extr = case_when(anio < 2004 ~ ccnn_oficial$vbp_extr[ccnn_oficial$anio == 2004] * vbp_criterio_ccnn_04,
                                    anio > 2012 ~ ccnn_oficial$vbp_extr[ccnn_oficial$anio == 2012]  * vbp_criterio_ccnn_12,
                                    T ~ vbp_extr) ,
    vbp_tot = case_when( anio < 2004 ~ ccnn_oficial$vbp_tot[ccnn_oficial$anio == 2004] * vbp_criterio_ccnn_04,
                           anio > 2012 ~ ccnn_oficial$vbp_tot[ccnn_oficial$anio == 2012]  * vbp_criterio_ccnn_12,
                           T ~ vbp_tot),
    ci_extr = case_when( (anio < 2004) | (anio > 2012)  ~ 
                                 vbp_extr *coef_tecnico_97$`Extracción de petróleo, gas, carbón y uranio`  ,
                               T ~ ci_extr),
    ci_tot = case_when( (anio < 2004) | (anio > 2012)  ~ 
                                 vbp_tot *coef_tecnico_97$`Extracción de petróleo, gas, carbón y uranio`  ,
                               T ~ ci_tot) ,
    va_tot = case_when(is.na(va_tot) ~ vbp_tot - ci_tot ,
                                   T ~ va_tot),
    va_extr = case_when(is.na(va_extr) ~ vbp_extr - ci_extr ,
                                   T ~ va_extr),
    ms_total = case_when(is.na(ms_total) ~ vbp_tot*mean_coef_ms$coef_ms_tot,
                                   T ~ ms_total) ,
    ms_extraccion = case_when(is.na(ms_extraccion) ~ vbp_tot*mean_coef_ms$coef_ms_extr,
                                   T ~ ms_extraccion), 
    ebe_total = case_when(is.na(ebe_total) ~ va_tot -ms_total,
                                   T ~ ebe_total),
    ebe_extraccion = case_when(is.na(ebe_extraccion) ~ va_tot -ms_total,
                                   T ~ ebe_extraccion))

## Criterio propio con emapalme
criterio_ccnn %>% 
  rename(unidad = unidad_valor_produccion) %>% 
  left_join(empalme_ccnn %>% 
              select(-c(vbp_criterio_ccnn_04, vbp_criterio_ccnn_12, vbp_tot))) %>% 
  mutate(
        # criterio propio
         ...
         va_tot_propio = vbp_tot_criterio_propio - ci_extr,
         ebe_tot_criterio_propio = va_tot_propio- ms_extr   )

glimpse(criterio_ccnn)
```
...
valor_total_produccion <- ccnn_oficial %>%  # no tiene descontado servicios
 mutate(fuente = "CCNN") %>% 
   full_join(pv_hidrocarburos_propia %>% 
              select(anio, consumo_k_fijo)) %>%  
  select(anio, unidad, fuente,vbp_extr,valor_agregado,
  ...
  bind_rows(vbp_ccnn %>% # CCNN Reestimado, tiene descontado serivicios
              ...
              rename(valor_bruto_produccion = vbp_extr_ccnn, 
                   consumo_intermedio = ci_extr_ccnn,
                   valor_agregado =va_extr_ccnn,
