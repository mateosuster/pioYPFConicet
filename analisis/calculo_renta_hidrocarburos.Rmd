...
# 2004 a 2019: indec
consumo_intermedio <- data.frame(anio = 1960:2020) %>%
  left_join(ccnn_oficial %>%  select(anio, vbp_tot, va_tot), by = "anio") %>% 
  left_join(consumo_intermedio_93 %>% select(anio, coef_tec_93)) %>% 
  mutate(coef_ci = case_when( anio >= 2004 ~ 1-(va_tot/vbp_tot),
                              anio < 2004 ~ coef_tec_93 ),
...
coeficientes <- list("Proporción de los servicios sobre extraccion" = servicio_s_extraccion,
                     ...
                     "Coeficiente consumo intermedio (Año)" =consumo_intermedio$anio,
                     "Coeficiente consumo intermedio (1- VA/VBP)" =consumo_intermedio$coef_ci)
