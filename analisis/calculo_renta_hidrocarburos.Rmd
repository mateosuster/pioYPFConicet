...
Criterio Propio con VBP de CCNN (*pv_hidrocarburos*)
  + Toma el VBP de criterio propio y para obtener el VA, se le descuenta el CI de extracción de CCNN restimado (sin embargo, como se dijo, el VBP no tiene descontado los servicios). Se utilizó tambien la MS estimada en CCNN restimado. El EBE surge como diferencia de las partidas. 



### Formulación matemática de los cálculos


* VBP con criterio propio 

$$VBP_{propia} = (Pext_{petróleo} * Q_{petróleo} + Pext_{gas} * Q_{gas}) * TCP$$

* VBP con criterio CCNN 
...
* $VBP_{CCNN}$ = Valor Bruto de la Producción,  estimación propia con criterio de las CCNN
...
vbp_va_ci_propia <- precios_y_cantidades %>% 
  mutate(crudo_mdo_interno = prod_crudo - expo_crudo,
         ...
         vbp_criterio_ccnn = ((crudo_mdo_interno*precio_crudo_mdoint+
                                 expo_crudo*precio_me_crudo
                               + gas_mdo_interno * precio_gas_mdoint 
                               +expo_gas*precio_externo_gas )*tcc)/10^6,
...
vbp_ccnn <- vbp_va_ci_ccnn %>% 
  select(anio,unidad, vbp_extraccion_hidrocarburos, 
         va_extraccion_hidrocarburos, ci_extraccion_hidrocarburos) %>% 
...
valor_total_produccion <- pv_hidrocarburos_ccnn %>%  # no tiene descontado servicios
 mutate(fuente = "CCNN") %>% 
  ...
  select(anio, unidad, fuente,valor_bruto_produccion,valor_agregado,
         consumo_intermedio, masa_salarial, excedente_bruto_explotacion, consumo_k_fijo) %>% 
  gather(key = variable,
         value = valor,
         4:ncol(.)) %>% 
  ...
  bind_rows(pv_hidrocarburos %>% 
              ...
              select(anio, unidad, fuente, valor_bruto_produccion = vbp, 
               ...
               gather(key = variable,
                      value = valor,
                      4:ncol(.))) %>% 
  bind_rows(vbp_va_ci_propia %>% # Criterio CCNN. No tiene descontado servicios 
              left_join(stock_estimado %>%  
                          select(-unidad),
                          # filter(variable == "ppye_estimado_bolsar"),
                        by = "anio") %>%
...
graf_valor_produccion <- valor_total_produccion %>% 
  ...
  labs(title = "Valor de la Producción hidrocarburífera", 
       ...
       y = "Millones de pesos de 2018")+
  facet_wrap(~variable)
ggplotly(graf_valor_produccion, width = 750, height = 400)
...
renta_hidrocarburifera_total <- renta_tg %>% 
  ...
  mutate(renta_con_union_tg = ppye* (tg_hidrocarburos - union_tg),
         ...
         proporcion_subsidios = subsidios/
           (excedente_bruto_explotacion - gcia_normal_hidrocarburos)) %>% 
  select(anio , unidad, everything(.))
...
renta_pbi_pv <- renta_dif_precios_crudo %>% 
  ...
  mutate(renta_dif_precios_gas = renta_dif_precios_gas /ipc_18,
         ...
         proporcion_subsidios = subsidios/(renta_dif_precios_gas+ renta_dif_precios_crudo +
           renta_sobrevaluacion_crudo + renta_sobrevaluacion_gas +
           regalias_total +  retenciones_crudo_jk)  ,
...
renta_pbi_pv[,c("anio","proporcion_subsidios")] %>% arrange(-anio)
```
...
costos <- produccion_total %>% 
  ...
  left_join(valor_total_produccion %>% 
              ...
              filter(variable %in% c("consumo_intermedio", "masa_salarial","consumo_k_fijo" ),
                     fuente%in% c("Criterio CCNN", "CCNN")) %>%
  ...
  mutate(ppye = (ppye*10^6)*ipc_18/tcc,
         ...
         costos_totales = (consumo_intermedio +masa_salarial + consumo_k_fijo),
...
writexl::write_xlsx(list(renta_total_pv_pbi = renta_pbi_pv,
                    ...
                    path = "resultados/argentina/renta_hidrocarburos_total.xlsx")


writexl::write_xlsx(renta_total_pbi_pv,
                    path = "resultados/argentina/renta_total_PxQ.xlsx")

writexl::write_xlsx(costos,
                    path = "resultados/argentina/costos.xlsx")
...
writexl::write_xlsx(list("TG indice flujo pozos"= tasa_ganancia_rama_flujo, 
                    ...
                    path = "resultados/argentina/tasa de ganancia y renta hidrocarburífera.xlsx")
