...
```{r}
produccion_provincia_crudo <- read_excel("data/regalias/produccion_provincia_crudo.xlsx" , 
    skip = 10) %>%
...
precio_provincia_crudo <- read_excel("data/regalias/precio_provincia_crudo.xlsx", skip = 12) %>% 
  rename(anio = "AÑO", mes = "MES") %>% 
  mutate(unidad = "USD/m3",
         variable = "Precio Mercado Interno del Crudo ",
         anio =zoo::na.locf(anio),
