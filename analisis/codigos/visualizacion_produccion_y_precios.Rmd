...
Los precios del mercado interno del gas natural poseen un comportamiento estable hasta fines de los '70, momento en el cual aumentan drásticamente. En 1980, en línea con el aumento del crudo, el precio del gas supera USD 1/MMBTU, aunque cae en los años subsiguientes; mientras que en la década de 1990 se mantiene estable y cae con la devaluación del 2002. A partir de ahí, presentó una clara tendencia ascendente hasta 2017, la cual se detuvo en el interregno 2011-2015.

```{r}
precio_gas_plt <-  ggplot(data[ variable == variables[8], ],
                          aes(anio, valor, color = fuente))+
  geom_line(alpha = 0.8)+
  labs(title = "Gráfico N° 4. Precios del mercado interno del gas natural. Argentina (1963 - 2020)",
       y = "USD/MMBTU", x = "Año",
       caption = "Fuente: elaboración propia en base a IDEE, Ministerio de Economía, Secretaría de Energía (Base Regalías) y Balances Contables de YPF")+
  # scale_y_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE))+
  scale_x_continuous(breaks = seq(1910, 2030, by = 10))
...
Notas: 
- Agregar dubai.
- conviene ajustar por inflación?

Con respecto a los precios de referencia internacional del petróleo crudo, tanto el precio Brent como WTI atravesaron una escalada inflacionaria a partir del cambio de sistema monetario internacional en 1973, la cual se detuvo en 1981. En ese año se ha detectado un punto de quiebre en la variable según el test de raíz unitaria de Zivot y Andrews (Grilli y Yang, 1988). El boom de las commodities del siglo XXI se ve afectado por la crisis de 2008 y luego se detiene y cae luego de 2011.   

```{r}
precio_ref_crudo_plt <-  ggplot(data[ variable == variables[9], ],
                          aes(anio, valor, color = fuente))+
  geom_line(alpha = 0.8)+
  labs(title = "Gráfico N° 4. Precios de referencia del mercado mundial del crudo (1946 - 2020)",
       y = "USD/MMBTU", x = "Año",
...
A grandes rasgos, los precios de exportación de crudo resultantes de distitnas fuentes, no presentan tanta diferencia con respecto al precio de exportación de crudo.
...
precio_expo_crudo_plt <-  ggplot(data[ variable == variables[10] 
                          ...
                          aes(anio, valor, color = fuente))+
  geom_line(alpha = 0.8)+
  labs(title = "Gráfico N° #. Precios de exportacion de crudo. Argentina (1964 - 2020)",
       y = "USD/barril", x = "Año",
...
brecha_precio_crudo_plt <-  ggplot(data[ 
                          ...
                          aes(anio, valor, color = fuente))+
  geom_line(alpha = 0.8)+
  labs(title = "Gráfico N° #. Precio de venta al mercado interno, exportacion y referencias del petróleo crudo (1960 - 2020)",
       y = "USD/barril", x = "Año",
...
precio_ref_gas_plt <-  ggplot(data[ (variable %in% variables[11:27])&
  ...
  geom_line(alpha = 0.8)+
  labs(title = "Gráfico N° #. Precios de referencia del gas natural de EEUU y Europa (1960 - 2020)",
       y = "USD/MMBTU", x = "Año",
...
precio_ref_gas_plt_2 <-  ggplot(data[ (variable %in% variables[11:27])&
                          ...
                          aes(anio, valor, color = variable))+
  geom_line(alpha = 0.8)+
  labs(title = "Gráfico N° #. Precios de exportación e importación del gas natural de EEUU y Europa (1984 - 2020)",
       y = "USD/MMBTU", x = "Año",
       caption = "Fuente: elaboración propia en base a EIA y BP")+
  # scale_y_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE))+
  scale_x_continuous(breaks = seq(1910, 2030, by = 10))
...
precio_expo_gas_plt <-  ggplot(data[ (variable %in% variables[33])
                          ...
                          aes(anio, valor, color = fuente))+
  geom_line(alpha = 0.8)+
  labs(title = "Gráfico N° #. Precios de exportación del gas natural. Argentina (1962 - 2020)",
       y = "USD/MMBTU", x = "Año",
...
precio_impo_gas_plt <-  ggplot(data[ (variable %in% variables[28:32]) &
  ...
  labs(title = "Gráfico N° #. Precios de importación del gas natural. Argentina (1962 - 2020)",
       y = "USD/MMBTU", x = "Año",
       caption = "Fuente: elaboración propia en base a Sec. de Energía, MECON, IDEE y UN-Comtrade")+
  # scale_y_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE))+
  scale_x_continuous(breaks = seq(1910, 2030, by = 10))
