...
renta_lab <- c("RTPG_PextQ sobre plusvalía", "RTPG_PextQ sobre PBI")
...
renta_lab <- c("RTPG_PextQ/PV", "RTPG_PextQ/PBI")
names(renta_lab) <- c("renta_pv" ,"renta_pbi")
...
ggplot() +
  ...
  theme_light()+
   theme(strip.text.x = element_text(size = 7), 
         axis.text.x = element_text(angle = 90, size=7))+# , vjust = 0.5, hjust=0.5))+
  labs(title = "Renta de la tierra del gas y el petróleo sobre PBI y plusvalía total", subtitle = "Argentina, Brasil, Bolivia y Venezuela (1997-2018)",
       y = "%", x="Año")+
  scale_y_continuous(labels=function(x) scales::percent(x, accuracy = 2) )+
  scale_fill_manual("Paises (RTPG_PextQ)", values=c(  "dodgerblue1",  "chartreuse2", "green4", "#CC6666"))+
    scale_color_manual(name = "Fuente",  values = c("black"))+
...
## Argentina
```{r}
arg_tg <- read_excel("../resultados/argentina/renta_de_la_tierra_hidrocarburifera_arg.xlsx", sheet = "tg_total") %>% 
...
## Venezuela
```{r}
vzl_tg <- read_excel("../data/venezuela/VARIABLES Renta Venezuela.xlsx", sheet = "Tasas de ganancia",
                     skip = 1) %>% 
...
## Argentina
```{r}
mecanismos_arg <- read_excel("../resultados/argentina/renta_de_la_tierra_hidrocarburifera_arg.xlsx", sheet = 1) %>% 
  select(-14) %>% 
...
## Venezuela
```{r}
mecanismos_vzl <-read_excel("../data/venezuela/VARIABLES Renta Venezuela.xlsx", sheet = "Renta" ) %>% 
  slice(-1) %>% 
  select(anio = "...1", renta_empresas = "Renta petrolera (TgP-Tgnop)" ,
         ...
         renta_diferencial_precios = "Venta de petróleo por debajo del precio internacional") %>% 
  ...
  mutate(pais = "Venezuela",
           unidad = "Millones de USD",
           valor_renta = valor_renta /valor_tc) 
mecanismos[is.na(mecanismos$tipo_renta), ]
```
...
### Comparacion
```{r}
mecanismos <- rbind(mecanismos_arg, mecanismos_bzl, mecanismos_vzl)
...
mecanismos %>% 
  ...
  # theme_bw()+
  theme_light()+
   theme(strip.text.x = element_text(size = 8), strip.text.y = element_text(size = 7))+
  labs(title = "Renta de la tierra del gas y el petróleo, por mecanismos de apropiacion", 
       subtitle = "Argentina, Brasil y Venezuela (1997-2018)",
       y = "Millones de USD", x="Año")+
    scale_y_continuous(labels=function(x) format(x, big.mark = ".", scientific = FALSE))+
  # scale_fill_manual("Paises", values=c(  "dodgerblue1",   "green4","#CC6666" )) +#, "chartreuse2" ))+
...
## Argentina
```{r}
x <-  read_excel("../resultados/argentina/renta_de_la_tierra_hidrocarburifera_arg.xlsx", sheet = "descuentos_vbp") %>% 
...
descuentos_arg <-  read_excel("../resultados/argentina/renta_de_la_tierra_hidrocarburifera_arg.xlsx", sheet = "criterio_propio") %>% 
...
pbi_vzl <- read_excel("../data/venezuela/VARIABLES Renta Venezuela.xlsx", sheet  = "PIB") %>% 
  select(anio = "...1", "valor agregado" = "PIB Petrolero")

pv_vzl <- read_excel("../data/venezuela/VARIABLES Renta Venezuela.xlsx", sheet  = "Plusvalía ") %>% 
  select(anio = "...1", "plusvalia" = "Plusvalía petrolera")

ci_vzl <- read_excel("../data/venezuela/VARIABLES Renta Venezuela.xlsx", sheet  = "CI") %>% 
  select(anio = "...1", "consumo intermedio" = "CI Petro")

w_vzl <- read_excel("../data/venezuela/VARIABLES Renta Venezuela.xlsx", sheet  = "Remuneraciones") %>% 
  select(anio = "...1", "masa salarial" = "Rem Petro")

consK_vzl <- read_excel("../data/venezuela/VARIABLES Renta Venezuela.xlsx", sheet  = "Consumo capital fijo") %>% 
...
renta_paises <- renta_paises %>% rename(RTPG_PextQ = renta_directo, RTPG_Mecanismos= renta_indirecto)
sobrepv_paises <- sobrepv_paises %>% mutate(
                         metodo= case_when(metodo == "directo" ~ "Renta de la tierra del petróleo y el gas calculada desde el producto de valor a precio internacional y valuda en moneda local por tipo de cambio de paridad",
