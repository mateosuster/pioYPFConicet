...
sobrepv_paises %>% 
  ...
  labs(title = "Renta de la tierra del gas y el petróleo sobre PBI", subtitle = "Argentina, Brasil y Venezuela (1997-2018)",
       y = "%", x="Año")+
...
sobrepib_paises %>% 
  filter(pais %in% c("Argentina", "Venezuela"), anio > "1996-06-06") %>% 
  # gather(., key = variable, value = valor, 2:3) %>% 
  ggplot(aes(anio, renta_pbi, color = fuente)) +
  geom_line(position = "dodge")+
  theme_light()+
   theme(strip.text.x = element_text(size = 8))+
  labs(title = "Renta de la tierra del gas y el petróleo sobre PBI", subtitle = "Argentina y Venezuela (1997-2018)",
       y = "%", x="Año")+
  scale_y_continuous(labels=function(x) scales::percent(x, accuracy = 2) )+
