...
renta_bil_fr = renta_hidrocarburos_fr %>%
  rename(renta_sobrevaluacion = expo_tc, 
         renta_diferencial_precios = dif_precios) %>% 
  reshape2::melt(id.vars = c("anio", "ipc_18", "unidad"),
                  variable.name = "tipo_renta",
                 value.name = "valor") %>% 
    left_join(tcp_anual_b) %>% 
    mutate(autor = "Fárfaro Ruiz y Bil",
...
renta_autores <- read_csv("../data/otros autores/renta_autores.csv", 
  ...
  select(-"...1") %>% 
  rbind(renta_bil_fr)%>%
  mutate(tipo_de_renta = case_when(tipo_de_renta=="subsidio_consumo_total" ~"subsidios",
                                   T~ tipo_de_renta) ,
         valor = case_when(
...
# data.table::fwrite(renta_autores, file = "../resultados/argentina/renta_autores.csv")
...
precio_mi_gas <- precio_regalias_mi_gas %>% 
  ...
  mutate(variable = "Precio del mercado interno del gas natural según distintas fuentes",
         precio_gas_mdoint = case_when(anio %in% 1963:1969|anio %in% 1989:1992 ~
                                           precio_mi_ypf_gas, 
...
data_comtrade  <-  read_csv("../data/un_comtrade/all_crude_gas_sict.csv") %>% 
  mutate(cantidad = case_when(Commodity == "Crude petroleum"~ 
                                  conversor_kgbbl_q(`Netweight (kg)`) ,
                                Commodity == "Gas, natural"~
                                  conversor_kgm3_q( `Netweight (kg)`)),
         unidad =case_when(Commodity == "Crude petroleum"~ 
                                  "barriles" ,
                                Commodity == "Gas, natural"~
                             "MMBTU"),
  ...
  select(anio = Period, Classification, 
...
comex_crudo_sesco <- comercio_exterior %>% 
  filter(str_detect(producto, "PETROLEO") | str_detect(producto, "Cuenca") |str_detect(producto, "Crudo importado"),
         unidad != "Ton") %>% 
  group_by(anio, variable, unidad) %>% 
...
expo_usd_crudo <- comex_crudo_sesco %>% 
  ...
  mutate(expo_crudo_mecon = expo_crudo_mecon *1e6,
...
impo_crudo = comex_crudo_sesco %>% 
  ...
  mutate(fuente = "Sec. Energía - SESCO",
  ...
  bind_rows(data_comtrade %>%
              ...
              pivot_longer(cols = c(USD,barriles), names_to = "unidad",
...
impo_gas = comex_gas %>% 
  ...
  bind_rows(data_comtrade %>%
              ...
              pivot_longer(cols = c(USD,m3), names_to = "unidad",
                           values_to = "valor", names_repair = "unique")) %>% 
  bind_rows(comex_hidrocarburos_mecon %>% 
              select(anio, valor = impo_MMm3_gas) %>% 
              mutate( fuente = "MECON", valor = valor*1e6, 
...
empleo_y_remuneraciones <-  masa_salarial_ccnn %>%
  ...
  codigo_variable = case_when(
    ...
    T ~NA_character_),
  valor = case_when(variable == "Masa salarial" ~ valor*1e6, 
                    T ~ valor),
  ...
  mutate(
    ...
    # renta_empresas =replace_na(renta_empresas,  0),
         ...
         renta_total = rowSums(cbind( renta_dif_precios_gas, renta_dif_precios_crudo ,
           renta_sobrevaluacion_crudo , renta_sobrevaluacion_gas ,
           renta_empresas, regalias_total ,  retenciones, subsidios*-1), na.rm = T),
...
```
```{r}
write.csv(tasa_ganancia_rama_stock , "../resultados/argentina/tasa_ganancia_rama_stock.csv",row.names=FALSE)
```
...
expo_q_crudo_dt[, codigo_variable := fcase( fuente==  "Sec. Energía - SESCO" , "X_crudo_sesco",
...
expo_usd_crudo[, codigo_variable := fcase( fuente==  "Sec. Energía - SESCO" , "X_crudo_sesco",
                                         fuente== "MECON", "X_crudo_mecon", 
                                         fuente== "INDEC", "X_crudo_indec", 
                                         fuente==   "UN-Comtrade", "X_crudo_comtrade" ) ]
#impo crudo
...
impo_crudo_dt[,  codigo_variable:=  fcase( fuente == "Sec. Energía - SESCO", "M_crudo_sesco",
...
expo_usd_gas[, codigo_variable := fcase( fuente==  "Sec. Energía - SESCO" , "X_gas_sesco",
                                         fuente== "MECON", "X_gas_mecon", 
                                         fuente== "INDEC", "X_gas_indec", 
                                         fuente==   "UN-Comtrade", "X_gas_comtrade" ) ]
...
impo_gas_dt[,  codigo_variable:=  fcase( fuente == "Sec. Energía - SESCO", "M_gas_sesco",
                                        fuente == "UN-Comtrade", "M_gas_comtrade")]
impo_gas_dt[, valor := fcase(unidad == "m3", conversor_m3MMBTU_q(valor) ,
                          unidad == "USD", valor)]
impo_gas_dt[, unidad := fcase(unidad == "m3", "MMBTU" ,
                           unidad == "USD", "USD")]
...
setnames(precio_mi_crudo,
         old = c("regalias", "mecon" , 
                 # "mecon_ccnn",
                 "ypf", "idee" ),
         new = c("Sec. Energía - Regalías", "MECON",
                 # "MECON/CCNN",
                 "YPF", "IDEE"  ))
...
setnames(precio_interno_gas_mmbtu_usd,
         old = c("anuario", "regalias", "mecon" ,
                 # "mecon_ccnn",
                 "ypf", "idee" ),
         new = c("Anuario de combustibles", "Sec. Energía - Regalías", "MECON",
                 # "MECON/CCNN", 
                 "YPF", "IDEE"  ))
...
all_raw_data = rbindlist(list(prod_merge_crudo ,prod_merge_gas_MMBTU, 
               ...
               empleo_y_remuneraciones),  use.names=TRUE)
...
write.csv(ipc_promedio, "../data/indices/ipc_annual_avg.csv", row.names = F)
...
write.csv(stock_segmentos, "../resultados/argentina/base_csv/stock_segmentos.csv", row.names = F)
write.csv(stock_balances_empresas, "../resultados/argentina/base_csv/stock_balances_empresas.csv", row.names = F)
write.csv(stock_ypf, "../resultados/argentina/base_csv/stock_ypf.csv", row.names = F)
