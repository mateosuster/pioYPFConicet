...
```{r}
renta_directo_pbi <- read_excel("../resultados/argentina/renta_de_la_tierra_hidrocarburifera_arg.xlsx", sheet = "RTPG_PextQ_vs_pib")
...
```{r}
renta_dif_precios_crudo =  read_excel("../resultados/argentina/renta_de_la_tierra_hidrocarburifera_arg.xlsx", sheet = "crudo_dif_pre_y_sv")
graf_dif_crudo <- renta_dif_precios_crudo %>%
  left_join(ipc_arg %>% select(anio, IPC_18)) %>%
  mutate(Rdifp_crudo = Rdifp_crudo/IPC_18) %>%
  filter(anio >1960) %>%
  mutate(Rdifp_crudo = Rdifp_crudo / 10^6) %>%
  ggplot(aes(anio, Rdifp_crudo))+
  geom_col(position = "dodge")+
  theme(legend.position = "bottom")+
...
```{r}
renta_diferencial_precios_gas = read_excel("../resultados/argentina/renta_de_la_tierra_hidrocarburifera_arg.xlsx", 
                                           sheet = "gas_dif_pre_y_sv")
graf_dif_gas <- renta_diferencial_precios_gas %>%
  left_join(ipc_arg %>% select(anio, IPC_18)) %>%
  mutate(Rdifp_gas = Rdifp_gas/IPC_18/ 10^6) %>%
  ggplot(aes(anio, Rdifp_gas)) +
  geom_col(position = "dodge")+
  theme(legend.position = "bottom")+
...
graf_tg_rama <- tasa_ganancia_rama_stock %>%
  ...
  # filter(anio >1960) %>%
  left_join(ipc_arg %>% select(anio, IPC_18)) %>%
  mutate(Rkindv = Rkindv/IPC_18) %>%
  ggplot(aes(anio, Rkindv, fill = stock_seleccionado))+
  geom_col(position ="dodge")+
  # geom_point()+
...
# renta_mecanismos[,c("anio","proporcion_subsidios")] %>% arrange(-anio)
renta_mecanismos  %>%
  mutate(peso_subsidios = Subs/Rtt,
         metodo = "Indirecto") %>%
         select(anio, metodo, peso_subsidios, Rtt) %>%
  # rbind(renta_directo %>%
  ...
  ggplot(aes(anio, peso_subsidios))+#, color = metodo))+
  geom_line()
```
...
```{r echo=FALSE}
#Renta tcp y tcc
graf_renta_total_usd <- renta_mecanismos %>%
...
```{r echo=FALSE}
#v pbi y pv
graf_renta_total_pv <- read_excel("../resultados/argentina/renta_de_la_tierra_hidrocarburifera_arg.xlsx", sheet = "RTPG_mecanismos_vs_pib") %>%
